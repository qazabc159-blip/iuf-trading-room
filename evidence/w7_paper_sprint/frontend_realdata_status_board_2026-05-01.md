...
- Files changed: `apps/api/src/server.ts`, `evidence/w7_paper_sprint/codex_scheduler_workspace_resolution_fix_2026-05-07.md`, Elva channel memo.
- Root cause addressed: token can exist while FinMind usage remains 0 if scheduler slug is wrong, because ticker resolution returns 0 rows before any FinMind API call.
- Checks: contracts/db/domain/integrations build PASS; api typecheck PASS; api build PASS; diff-check PASS with CRLF warning only; code-only stop-line grep PASS.
- Stop-lines: no token value, no Railway secret edit, no migration/schema/destructive DB, no KGI/broker write-side, no order route, no fake-live, no buy/sell, no strategy metric promotion.
- Deploy verify needed: confirm API logs show `[schedulers] Using workspace "..."`; FinMind official/request counters should increase after a scheduler tick if token and companies exist.
...
### Codex cycle (2026-05-03 11:58 Taipei) - global density and edge-spacing repair
- Files changed: updated `apps/web/app/globals.css`, `apps/web/components/DataSourceBadge.tsx`, `apps/web/app/login/page.tsx`, `apps/web/app/register/page.tsx`, and this board.
- Endpoints / data behavior: no endpoint contract changed. This is a presentation-only repair for the operator complaint that labels, icons, controls, and panel titles were too close to borders and separator lines. Login/register, dashboard, company detail, K-line, portfolio ticket, signals, themes, and market-intel still use the same authentication/read/paper endpoints and fail closed when data is unavailable.
- Behavior change: widened global page gutters, sidebar hit rows, nav icons, panel headers, table rows, dashboard rows, theme rows, signal rows, K-line toolbar/meta rows, company info sections, company source rows, paper-ticket cards, modal field boxes, radar headers, market-intel rows, and the fixed data-source badge. Removed the old inline `paddingTop: 0` from login/register form headers so public auth pages also follow the same breathing-room rule.
- Tests: `git diff --check -- apps/web/app/globals.css apps/web/components/DataSourceBadge.tsx apps/web/app/login/page.tsx apps/web/app/register/page.tsx` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS. Local production Playwright QA at 1365px on `127.0.0.1:3072` swept `/login`, `/register`, `/`, `/themes`, `/companies`, `/companies/1101`, `/portfolio`, `/signals`, and `/market-intel`: all returned 200 with 0 horizontal overflow, 0 narrow vertical text stacks, 0 padding-threshold failures, 0 page errors, 0 console errors, and 0 visible mojibake/raw diagnostic hits. Evidence remains local at `evidence/w7_paper_sprint/local_visual_qa_pass52_global_density_rerun_2026-05-03/`.
- Blockers: production authenticated visual smoke is pending after merge/deploy. KGI `libCGCrypt.so` remains the live-submit blocker only. Stop-lines respected: no broker write, no migration 0020, no Railway secrets, no live submit, no KGI SDK/write-side, no destructive DB action, and no deferred RSS/news/commercial-data feature.

### Codex cycle (2026-05-03 12:07 Taipei) - root flex overflow guard
- Files changed: updated `apps/web/app/layout.tsx` and this board.
- Endpoints / data behavior: no endpoint contract changed. This is a root layout-only CSS/flex containment fix.
- Behavior change: the main content wrapper next to the fixed sidebar now has `minWidth: 0`, so pages with wide semantic tables can shrink within the viewport instead of pushing the whole app 40-50px past the right edge. This directly fixes the production `/themes` overflow caught after pass52 deployment.
- Tests: production pass52 smoke found `/themes` overflow 49px while all other checked routes were 0. `git diff --check -- apps/web/app/layout.tsx evidence/w7_paper_sprint/frontend_realdata_status_board_2026-05-01.md` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS. Local production Playwright rerun at 1365px on `127.0.0.1:3072/themes` returned 200 with 0 horizontal overflow and 0 narrow vertical stacks. Evidence remains local at `evidence/w7_paper_sprint/local_visual_qa_pass53_flex_overflow_2026-05-03/`.
- Blockers: production authenticated visual smoke will rerun after merge/deploy. KGI `libCGCrypt.so` remains the live-submit blocker only. Stop-lines respected: no broker write, no migration 0020, no Railway secrets, no live submit, no KGI SDK/write-side, no destructive DB action, and no deferred RSS/news/commercial-data feature.

### Codex cycle (2026-05-03 12:19 Taipei) - order segmented-control no-wrap repair
- Files changed: updated `apps/web/app/companies/[symbol]/PaperOrderPanel.tsx` and this board.
- Endpoints / data behavior: no endpoint contract changed. The company-side paper ticket still sends explicit Taiwan units (`SHARE` = odd-lot shares, `LOT` = board lots = 1,000 shares) through the existing paper-preview path only; no broker write, KGI SDK/write-side, Railway secret, migration, RSS/news/commercial-data feature, or destructive DB path was touched.
- Behavior change: segmented order controls now wrap by button group instead of wrapping inside the label. This prevents narrow side panels from rendering labels such as buy/sell, market/limit, stop/stop-limit, odd-lot, and board-lot as vertical stacked text. Buttons keep larger hit areas and preserve the zero-fake/live-submit stop-line.
- Tests: `git diff --check -- apps/web/app/companies/[symbol]/PaperOrderPanel.tsx` PASS with CRLF warning only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS. Local production server on `127.0.0.1:3072` is available; local authenticated rendering is blocked by local API/session mismatch, so final visual verification will run on production immediately after merge/deploy. Production pass53 smoke before this patch verified 9 routes at 1365px with 0 horizontal overflow, 0 narrow vertical stacks, 0 console/page errors, and 0 targeted raw diagnostic hits. Evidence: `evidence/w7_paper_sprint/production_smoke_pass53_flex_overflow_2026-05-03/`.
- Blockers: production visual smoke is required after merge/deploy to verify the button no-wrap fix against real company/portfolio data. KGI `libCGCrypt.so` remains the live-submit blocker only. Stop-lines respected.

### Codex cycle (2026-05-03 12:46 Taipei) - order-ticket narrow column repair
- Files changed: updated `apps/web/app/companies/[symbol]/PaperOrderPanel.tsx`, `apps/web/components/portfolio/OrderTicket.tsx`, and this board.
- Endpoints / data behavior: no endpoint contract changed. Company-side and trading-room paper tickets still use the existing paper preview/submit contracts with explicit Taiwan units (`SHARE` = odd-lot shares, `LOT` = board lots = 1,000 shares). No broker write, live submit, KGI SDK/write-side, Railway secret, migration, RSS/news/commercial-data feature, or destructive DB path was touched.
- Behavior change: company-side order form no longer forces direction/type/unit controls into a two-column side panel. The portfolio ticket segmented controls now wrap by whole button while preserving no-break Traditional Chinese labels, so labels such as buy/sell, market/limit, stop/stop-limit, odd-lot, and board-lot do not collapse into vertical text in narrow columns.
- Tests: `git diff --check -- apps/web/app/companies/[symbol]/PaperOrderPanel.tsx apps/web/components/portfolio/OrderTicket.tsx evidence/w7_paper_sprint/frontend_realdata_status_board_2026-05-01.md` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS. Production visual smoke will run after merge/deploy.
- Blockers: none for this scoped UI patch. KGI `libCGCrypt.so` remains the live-submit blocker only. Stop-lines respected.

### Codex cycle (2026-05-03 13:00 Taipei) - production sweep raw-copy follow-up
- Files changed: updated `apps/web/lib/operator-copy.ts`, `apps/web/lib/content-draft-view.ts`, mobile kill-switch spacing in `apps/web/app/globals.css`, and this board.
- Endpoints / data behavior: no endpoint contract changed. This is display-only cleanup for existing brief/draft/mobile read pages; no broker write, live submit, KGI SDK/write-side, Railway secret, migration, RSS/news/commercial-data feature, or destructive DB path was touched.
- Behavior change: full production sweep after pass55 found no horizontal overflow, no mojibake, and no vertical stacks on the main pages, but `/briefs` still exposed `Audit Trail`, `/drafts` still exposed lowercase `deprecated`, and `/m/kill` had mobile status labels that could be misdetected as narrow stacks. This patch translates the remaining Audit Trail/deprecated source terms, maps draft target tables to Traditional Chinese labels, and gives kill-mode status labels a no-wrap reserved column.
- Tests: `git diff --check -- apps/web/lib/operator-copy.ts apps/web/lib/content-draft-view.ts apps/web/app/globals.css evidence/w7_paper_sprint/frontend_realdata_status_board_2026-05-01.md` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS. Focused production smoke will run after merge/deploy.
- Blockers: none for this scoped UI/text patch. KGI `libCGCrypt.so` remains the live-submit blocker only. Stop-lines respected.

### Codex cycle (2026-05-03 13:17 Taipei) - draft queue readability pass
- Files changed: updated `apps/web/app/drafts/page.tsx`, `apps/web/app/briefs/page.tsx`, and this board.
- Endpoints / data behavior: no endpoint contract changed. Drafts still read the same content-draft endpoint with the same backend limit; the visible table now presents the newest 30 rows first and discloses how many additional rows remain in the database/back-office queue. Brief history still reads the same daily-brief endpoint.
- Behavior change: the content draft route no longer renders a 100-row full-page data dump by default. It keeps the queue truthful but makes the first screen more usable. Brief history now maps internal producers such as `openalice` and `worker` to Traditional Chinese operator labels.
- Tests: `git diff --check -- apps/web/app/drafts/page.tsx apps/web/app/briefs/page.tsx evidence/w7_paper_sprint/frontend_realdata_status_board_2026-05-01.md` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS. Focused production smoke will run after merge/deploy.
- Blockers: none for this scoped UI/readability patch. KGI `libCGCrypt.so` remains the live-submit blocker only. Stop-lines respected.

### Codex cycle (2026-05-03 13:34 Taipei) - border breathing and fixed-badge clearance
- Files changed: updated `apps/web/app/globals.css`, `apps/web/components/DataSourceBadge.tsx`, `apps/web/app/companies/[symbol]/PaperOrderPanel.tsx`, `apps/web/components/portfolio/OrderTicket.tsx`, and this board.
- Endpoints / data behavior: no endpoint contract changed. This is a presentation-only spacing pass. Company-side and trading-room paper tickets still keep explicit Taiwan unit semantics: `SHARE` means odd-lot shares and `LOT` means board lots of 1,000 shares. No broker write, live submit, KGI SDK/write-side, Railway secret, migration, RSS/news/commercial-data feature, or destructive DB path was touched.
- Behavior change: widened the breathing room around bordered panels, panel headers, ASCII section headers, compact terminal notes, K-line toolbar/meta rows, company info sections, field boxes, source lines, market preview cards, paper-ticket source bars, key/value rows, guard rows, history rows, and order rows. The fixed data-source badge now sits farther from the right/bottom edge, has larger internal padding, and no longer captures pointer events, so it reads as a status indicator instead of crowding operational content.
- Tests: production pre-check on current main at 1365px swept `/`, `/companies/1101`, and `/portfolio` after login with 0 horizontal overflow and 0 detected low-padding bordered elements; screenshots saved under `evidence/w7_paper_sprint/production_smoke_pass60_border_spacing_2026-05-03/`. Local checks for this patch: `git diff --check -- apps/web/app/globals.css apps/web/components/DataSourceBadge.tsx apps/web/app/companies/[symbol]/PaperOrderPanel.tsx apps/web/components/portfolio/OrderTicket.tsx` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Blockers: production post-merge visual smoke is required after deploy. KGI `libCGCrypt.so` remains the live-submit blocker only. Stop-lines respected.

### Codex cycle (2026-05-03 13:45 Taipei) - dashboard first-screen market tape
- Files changed: updated `apps/web/app/page.tsx` and this board.
- Endpoints / data behavior: no endpoint contract changed. The dashboard still uses the existing `getMarketDataOverview({ includeStale: true, topLimit: 5 })` route and does not add any RSS/news/commercial-data source.
...
### Codex cycle (2026-05-03 14:18 Taipei) - demo-critical border clearance
- Files changed: updated `apps/web/app/globals.css`, `apps/web/app/companies/[symbol]/PaperOrderPanel.tsx`, and this board.
- Endpoints / data behavior: no endpoint contract changed. No broker write, live submit, KGI SDK/write-side, Railway secret, migration, RSS/news/commercial-data feature, or destructive DB path was touched. This is a demo-critical UI fix for company detail / K-line / simulated order surfaces.
- Behavior change: page content now reserves a larger right-side safe gutter on desktop so floating browser/status widgets do not sit on top of bordered panels. Global bordered buttons now have real vertical padding instead of relying only on min-height, K-line controls and pending chips have more breathing room, company info sections keep bottom padding, and the individual-company simulated-order banner/status notes no longer press text close to their borders.
- Tests: production pre-check on deployed `46ba264` swept `/companies/1101`, `/portfolio`, `/register`, and `/admin/invites` at 1365px after login with 0 horizontal overflow and 0 console/page errors; screenshots saved under `evidence/w7_paper_sprint/production_smoke_pass63_border_clearance_pre_2026-05-03/` and scroll-position company screenshots under `evidence/w7_paper_sprint/production_smoke_pass63_company_scroll_pre_2026-05-03/`. Local checks for this patch: `git diff --check -- apps/web/app/globals.css apps/web/app/companies/[symbol]/PaperOrderPanel.tsx` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web build` PASS; initial `typecheck` hit a transient `.next/types` race while build was running, rerun `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS.
- Blockers: production post-merge visual smoke required after deploy. KGI `libCGCrypt.so` remains the live-submit blocker only. Stop-lines respected.

### Codex cycle (2026-05-03 14:36 Taipei) - responsive sidebar and mobile overflow repair
- Files changed: updated `apps/web/app/layout.tsx`, `apps/web/app/globals.css`, `apps/web/components/portfolio/OrderTicket.tsx`, `apps/web/components/portfolio/KillSwitch.tsx`, and this board.
- Endpoints / data behavior: no endpoint contract changed. This is a demo-critical UI/layout repair only. Paper tickets still keep explicit Taiwan unit semantics: `SHARE` means odd-lot shares and `LOT` means board lots of 1,000 shares. No broker write, live submit, KGI SDK/write-side, Railway secret, migration 0020, RSS/news/commercial-data feature, or destructive DB path was touched.
- Behavior change: root layout now uses CSS classes instead of inline flex so responsive rules can work cleanly. At <=1000px the left sidebar becomes a sticky top horizontal navigation with icons and Traditional Chinese labels, instead of squeezing the whole app into a 150px side rail. Quote/KPI strips no longer create page-level horizontal scroll on mobile. Portfolio order cards and trading-mode cards now auto-fit to one/two columns on narrow screens, preventing labels and mode cards from stacking Chinese text one character per line.
- Tests: `git diff --check -- apps/web/app/layout.tsx apps/web/app/globals.css apps/web/components/portfolio/OrderTicket.tsx apps/web/components/portfolio/KillSwitch.tsx` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS. Clean local production Playwright QA on `127.0.0.1:3093` swept `/`, `/themes`, `/companies/1101`, and `/portfolio` at both 1365px desktop and 390px mobile: all returned 200 with 0 horizontal overflow, 0 narrow vertical text stacks, 0 visible raw diagnostic strings, 0 page errors, and 0 console errors. Evidence remains local at `evidence/w7_paper_sprint/local_visual_qa_pass64_mobile_sidebar_2026-05-03_final2/`.
- Blockers: production post-merge visual smoke required after deploy. KGI `libCGCrypt.so` remains the live-submit blocker only. Stop-lines respected.

### Codex cycle (2026-05-03 14:48 Taipei) - mobile production row stacking repair
- Files changed: updated `apps/web/app/globals.css` and this board.
- Endpoints / data behavior: no endpoint contract changed. This is a demo-critical layout-only repair for production rows/cards on mobile. No broker write, live submit, KGI SDK/write-side, Railway secret, migration 0020, RSS/news/commercial-data feature, or destructive DB path was touched.
- Behavior change: on narrow screens, production row surfaces (`themes`, dashboard theme rows, telex/news rows, signal ledger rows, strategy idea rows, limits/positions, and duplicate-company rows) now hide desktop-only table headers and stack into single-column readable cards. Child cells no longer inherit desktop alignment, nowrap, or ellipsis rules that squeezed Chinese/English text into vertical one-character columns. Source badges are hidden on mobile to preserve content width.
- Tests: `git diff --check -- apps/web/app/globals.css` PASS; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS. Local production Playwright QA on `127.0.0.1:3094` swept `/`, `/themes`, `/companies/1101`, and `/portfolio` at 390px mobile: all returned 200 with 0 horizontal overflow, 0 narrow vertical text stacks, 0 visible raw diagnostic strings, and 0 page errors. Evidence remains local at `evidence/w7_paper_sprint/local_visual_qa_pass65_mobile_rows_2026-05-03/`.
- Blockers: production post-merge mobile visual smoke required after deploy. KGI `libCGCrypt.so` remains the live-submit blocker only. Stop-lines respected.

### Codex cycle (2026-05-03 15:07 Taipei) - residual mobile table and quote-card repair
- Files changed: updated `apps/web/app/globals.css`, `apps/web/components/portfolio/OrderTicket.tsx`, and this board.
- Endpoints / data behavior: no endpoint contract changed. This is a demo-critical mobile layout/readability repair only. Paper-ticket unit semantics remain explicit: `SHARE` means odd-lot shares and `LOT` means board lots of 1,000 shares. No broker write, live submit, KGI SDK/write-side, Railway secret, migration 0020, RSS/news/commercial-data feature, or destructive DB path was touched.
- Behavior change: company financial tables now render as mobile cards below 760px instead of keeping a six-column desktop table that widened the company page. The portfolio quote preview grid now auto-fits cards at a usable minimum width, preventing bid/ask text from collapsing into one-character vertical stacks.
- Tests: `git diff --check -- apps/web/app/globals.css apps/web/components/portfolio/OrderTicket.tsx` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS. Local production Playwright QA on `127.0.0.1:3095` swept `/companies/1101` and `/portfolio` at 390px mobile: both returned 200 with 0 horizontal overflow, 0 narrow vertical text stacks, 0 console errors, and 0 page errors. Evidence remains local at `evidence/w7_paper_sprint/local_visual_qa_pass66_mobile_residual_2026-05-03/`.
- Blockers: production post-merge mobile visual smoke required after deploy. KGI `libCGCrypt.so` remains the live-submit blocker only. Stop-lines respected.

### Codex cycle (2026-05-03 15:23 Taipei) - company main-column shrink fix
- Files changed: updated `apps/web/app/globals.css` and this board.
- Endpoints / data behavior: no endpoint contract changed. This is a CSS containment fix only. No broker write, live submit, KGI SDK/write-side, Railway secret, migration 0020, RSS/news/commercial-data feature, or destructive DB path was touched.
- Behavior change: the company detail main/side grid columns now use `minmax(0, 1fr)` and child panels are capped at `max-width: 100%`. This prevents K-line and company-master panels from using intrinsic content width to push the mobile company page 18px wider than the viewport.
- Tests: production pass66 smoke identified only `/companies/1101` mobile residual overflow: K-line/company-master panels measured `left 18 / right 408 / width 390` inside a 390px viewport. Local checks for this patch: `git diff --check -- apps/web/app/globals.css` PASS; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS. Local production Playwright QA on `127.0.0.1:3096/companies/1101` returned 200 with 0 horizontal overflow, 0 overflowing non-sidebar elements, 0 console errors, and 0 page errors. Evidence remains local at `evidence/w7_paper_sprint/local_visual_qa_pass67_company_column_2026-05-03/`.
- Blockers: production post-merge mobile visual smoke required after deploy. KGI `libCGCrypt.so` remains the live-submit blocker only. Stop-lines respected.

### Codex cycle (2026-05-03 15:46 Taipei) - mobile card layout and border-clearance sweep
- Files changed: updated `apps/web/app/globals.css` and this board.
- Endpoints / data behavior: no endpoint contract changed. This is a demo-critical responsive layout fix only. It does not touch broker write, live submit, KGI SDK/write-side, Railway secrets, migration 0020, RSS/news/commercial-data features, or destructive DB paths.
- Behavior change: quote and lab two-column grids now collapse to one readable column below tablet width, preventing the K-line/source panels and quote blocked-state cards from squeezing text into vertical columns. Mobile panel headers now stack title/source metadata with real breathing room. Lab rows, brief-history rows, and draft rows now render as mobile cards with field labels instead of retaining desktop table widths. Shared mobile panels, state notes, tickets, and field boxes keep larger left/right gutters so text and icons do not sit tight against borders.
- Tests: `git diff --check -- apps/web/app/globals.css` PASS; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS. Local production Playwright QA on `127.0.0.1:3098` at 390px swept `/quote?symbol=2330`, `/lab`, `/briefs`, `/drafts`, `/companies/1101`, and `/portfolio` with a transient authenticated session: all returned 200 with 0 horizontal overflow, 0 body overflow, 0 overflowing content elements, 0 narrow vertical text stacks, 0 text-too-close-to-border findings, 0 targeted raw diagnostic hits, 0 console errors, and 0 page errors. Evidence remains local at `evidence/w7_paper_sprint/local_visual_qa_pass68_mobile_cards_2026-05-03/`.
- Blockers: production post-merge mobile visual smoke required after deploy. KGI `libCGCrypt.so` remains the live-submit blocker only. Stop-lines respected; QA credentials were used only in process memory and not written to repo/evidence.

### Codex cycle (2026-05-03 16:06 Taipei) - content draft ledger readability
- Files changed: updated `apps/web/app/drafts/page.tsx`, `apps/web/lib/content-draft-view.ts`, `apps/web/app/globals.css`, and this board.
- Endpoints / data behavior: no endpoint contract changed. Drafts still read the existing content-draft endpoint and link to the existing admin draft detail route. No broker write, live submit, KGI SDK/write-side, Railway secrets, migration 0020, RSS/news/commercial-data features, or destructive DB paths were touched.
- Behavior change: the content draft ledger no longer presents as a cramped database dump. The row target column now maps `company_notes` and `theme_summaries` to Traditional Chinese labels, the redundant view-button column was removed because each row is already the actionable link, and desktop rows now use a wider readable layout with larger titles plus three-line clipped excerpts. Mobile card behavior from the previous pass remains intact.
- Tests: `git diff --check -- apps/web/app/globals.css apps/web/app/drafts/page.tsx apps/web/lib/content-draft-view.ts` PASS; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS. Local production Playwright QA on `127.0.0.1:3099/drafts` at 1365px and 390px returned 200 with 0 horizontal overflow, 0 body overflow, 0 overflowing content elements, 0 narrow vertical text stacks, 0 raw `company notes` / `theme summaries` leakage, 0 targeted raw diagnostics, 0 console errors, and 0 page errors. Evidence remains local at `evidence/w7_paper_sprint/local_visual_qa_pass69_drafts_readability_2026-05-03/`.
- Blockers: production post-merge full-route smoke required after deploy. KGI `libCGCrypt.so` remains the live-submit blocker only. Stop-lines respected.

### Codex cycle (2026-05-03 16:31 Taipei) - ops hydration stability
- Files changed: updated `apps/web/app/ops/page.tsx` and this board.
- Endpoints / data behavior: no endpoint contract changed. The operations page still reads the same operations snapshot endpoint and stays read-only. No broker write, live submit, KGI SDK/write-side, Railway secrets, migration 0020, RSS/news/commercial-data features, or destructive DB paths were touched.
- Behavior change: operations-page timestamps now render with an explicit `Asia/Taipei` timezone and deterministic hour/minute fields. This targets the production-only React hydration error seen on `/ops` in the full-route sweep while keeping all visible copy in Traditional Chinese.
- Tests: production pass70 full-route sweep found only `/ops` page errors (`React error #418`) on desktop and mobile; no overflow or narrow text stacks were found. Local checks for this patch: `git diff --check -- apps/web/app/ops/page.tsx` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS. Local production Playwright QA on `127.0.0.1:3100/ops` at 1365px and 390px with a transient authenticated session returned 200 with 0 console errors and 0 page errors. Evidence remains local at `evidence/w7_paper_sprint/local_visual_qa_pass70_ops_hydration_2026-05-03/`.
- Blockers: production post-merge full-route smoke required after deploy to confirm `/ops` no longer emits React hydration errors. KGI `libCGCrypt.so` remains the live-submit blocker only. Stop-lines respected; QA credentials were used only in process memory and not written to repo/evidence.

### Codex cycle (2026-05-03 17:03 Taipei) - design density repair for dashboard, plans, company detail, and order ticket
- Files changed: updated `apps/web/app/globals.css`, `apps/web/app/plans/page.tsx`, `apps/web/app/companies/[symbol]/PaperOrderPanel.tsx`, and this board.
- Endpoints / data behavior: no endpoint contract changed. Dashboard, plans, company detail, K-line, and paper-order surfaces still read the existing production endpoints and keep the 4-state truth model. No broker write, live submit, KGI SDK/write-side, Railway secrets, migration 0020, RSS/news/commercial-data feature, or destructive DB path was touched. Taiwan unit semantics remain explicit: `SHARE` is odd-lot shares and `LOT` is board lots of 1,000 shares.
- Behavior change: the dashboard hero now has a distinct trading-room band treatment instead of another plain boxed table. Global HUD panels, ASCII headers, terminal notes, company info sections, K-line toolbars, K-line insufficient-data blocks, company KPI rows, and simulated-order controls were tightened so text has breathing room without turning every page into huge identical boxes. The transaction-plan page no longer uses a brittle six-column desktop row; plans now render as readable cards with separate status, plan text, risk/reward, review, and updated-at fields. The company detail K-line controls are compact again above the chart, and the company-detail simulated-order form uses smaller input/segment/button density while preserving the odd-lot/board-lot guard.
- Tests: `git diff --check -- apps/web/app/plans/page.tsx apps/web/app/companies/[symbol]/PaperOrderPanel.tsx apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web build` PASS; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS after rerun (first run raced with `.next/types` regeneration while build was running). Local production Playwright QA on `127.0.0.1:3102` with a transient authenticated session swept `/`, `/plans`, `/companies/1101`, and `/portfolio` at 1365px desktop and 390px mobile: all returned 200 with 0 horizontal overflow, 0 narrow vertical text stacks, 0 targeted raw diagnostic hits, 0 console errors, and 0 page errors. Evidence remains local at `evidence/w7_paper_sprint/local_visual_qa_pass72_design_repair_2026-05-03/`.
- Blockers: production post-merge visual smoke is required after deploy because the final judgment is the live site with Railway env and production data. KGI `libCGCrypt.so` remains the live-submit blocker only. Stop-lines respected; QA credentials were used only in process memory and not written to repo/evidence.

### Codex cycle (2026-05-03 17:34 Taipei) - portfolio ticket density and panel stiffness repair
- Files changed: updated `apps/web/components/portfolio/OrderTicket.tsx`, `apps/web/app/globals.css`, and this board.
- Endpoints / data behavior: no endpoint contract changed. The trading-room ticket still uses the existing paper preview/order endpoints and the same production quote/K-line read path. Taiwan unit semantics remain explicit and unchanged: `SHARE` means odd-lot shares and `LOT` means board lots of 1,000 shares. No broker write, live submit, KGI SDK/write-side, Railway secrets, migration 0020, RSS/news/commercial-data feature, or destructive DB path was touched.
- Behavior change: the portfolio trading room no longer renders the quote preview, order draft, and risk preview as oversized boxed blocks. The paper ticket now uses a tighter trading-ticket density, smaller segmented controls, smaller field height, a slimmer source strip, compact quote preview cards, and a two-column desktop execution layout at 1181-1450px instead of collapsing too early into a single stacked template. Shared panel/header padding was reduced so dashboard/plans/company/portfolio pages keep breathing room without the repeated heavy-card look.
- Tests: `git diff --check -- apps/web/components/portfolio/OrderTicket.tsx apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS. Local production Playwright QA on `127.0.0.1:3103` with a transient authenticated session swept `/`, `/portfolio`, `/companies/1101`, and `/plans` at 1365px desktop and 390px mobile: all returned 200 with 0 horizontal overflow, 0 narrow vertical text stacks, 0 console errors, and 0 page errors. Evidence remains local at `evidence/w7_paper_sprint/local_visual_qa_pass73_portfolio_density_2026-05-03_final/`.
- Blockers: production post-merge visual smoke is required after deploy. KGI `libCGCrypt.so` remains the live-submit blocker only. Stop-lines respected; QA credentials were used only in process memory and not written to repo/evidence.

### Codex cycle (2026-05-03 23:36 Taipei) - dashboard command deck repair
- Files changed: updated `apps/web/app/page.tsx`, `apps/web/app/globals.css`, and this board.
- Endpoints / data behavior: no endpoint contract changed. The dashboard still reads existing production market, watchlist, theme, idea, signal, and disclosure endpoints through the existing 4-state truth model. No broker write, live submit, KGI SDK/write-side, Railway secret, migration 0020, RSS/news/commercial-data feature, or destructive DB path was touched.
- Behavior change: the dashboard first screen no longer starts as another plain boxed table. It now opens with a distinct command-deck hero: core dashboard counts are inline, source health is shown as a compact rail with real updated timestamps and state pills, and the real market strip follows immediately below. This keeps the homepage visually different from the rest of the app while preserving truthful LIVE/EMPTY/BLOCKED/HIDDEN behavior.
- Tests: `git diff --check -- apps/web/app/page.tsx apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS. Local production Playwright QA on `127.0.0.1:3104` swept `/`, `/themes`, and `/signals` at 1365px desktop and 390px mobile: all returned 200 with 0 horizontal overflow, 0 narrow vertical text stacks, 0 targeted raw diagnostic hits, 0 console errors, and 0 page errors. Evidence remains local at `evidence/w7_paper_sprint/local_visual_qa_pass74_dashboard_mosaic_2026-05-03_final/`.
- Blockers: production post-merge visual smoke is required after deploy. The freeze directive file `evidence/w7_paper_sprint/TO_CODEX_freeze_breach_revert_and_banner_2026-05-02.md` is absent in this worktree, so this cycle followed the latest user-provided freeze/stop-line instructions directly. KGI `libCGCrypt.so` remains the live-submit blocker only. Stop-lines respected.

### Codex cycle (2026-05-03 18:42 Taipei) - company detail K-line and main panel density
- Files changed: updated `apps/web/app/companies/[symbol]/OhlcvCandlestickChart.tsx`, `apps/web/app/globals.css`, and this board.
- Endpoints / data behavior: no endpoint contract changed. Company detail still reads the existing company master, OHLCV, paper preview/order, and source-status surfaces. K-line still uses only official OHLCV rows; no mock fallback was added. The company-detail simulated order panel preserves Taiwan unit semantics: `SHARE` means odd-lot shares and `LOT` means board lots of 1,000 shares. No broker write, live submit, KGI SDK/write-side, Railway secret, migration 0020, RSS/news/commercial-data feature, or destructive DB path was touched.
- Behavior change: the company detail page now defaults the K-line to `全部` official OHLCV instead of a narrower range, lowers the chart height, and makes the day/week/month/range controls a compact toolbar instead of large blocks. Pending intraday intervals are now a truthful status line (`等待 KGI`) rather than button-like chips. The company hero loses the heavy full-box treatment, the main grid tightens, and company-master/validation/exposure sections now render as compact information rows so the page no longer wastes large empty blocks before useful data.
- Tests: `git diff --check -- apps/web/app/companies/[symbol]/OhlcvCandlestickChart.tsx apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS. Local production Playwright QA on `127.0.0.1:3105` with `NEXT_PUBLIC_API_BASE_URL=https://api.eycvector.com` and a transient authenticated session swept `/companies/1101`, `/portfolio`, and `/plans` at 1365px desktop and 390px mobile: all returned 200 with 0 horizontal overflow, 0 narrow vertical text stacks, 0 targeted raw diagnostic hits, 0 console errors, and 0 page errors. Evidence remains local at `evidence/w7_paper_sprint/local_visual_qa_pass75_company_realdata_2026-05-03_final/`.
- Blockers: production post-merge visual smoke is required after deploy. KGI `libCGCrypt.so` remains the live-submit blocker only. Stop-lines respected; QA credentials were used only in process memory and not written to repo/evidence.

### Codex cycle (2026-05-03 19:04 Taipei) - plans and daily brief workbench repair
- Files changed: updated `apps/web/app/plans/page.tsx`, `apps/web/app/briefs/page.tsx`, `apps/web/app/globals.css`, and this board.
- Endpoints / data behavior: no endpoint contract changed. `/plans` still reads the existing plans, companies, themes, signals, briefs, reviews, and strategy-ideas endpoints. `/briefs` now explicitly uses dynamic rendering so authenticated production requests read the real daily-brief endpoint instead of a stale build-time blocked state. No broker write, live submit, KGI SDK/write-side, Railway secret, migration 0020, RSS/news/commercial-data feature, or destructive DB path was touched.
- Behavior change: the transaction-plan page no longer renders as one generic table template. It now has a distinct decision-workbench layout: left-side official plan/idea flow, right-side daily-brief context, review notes, and signal context. Text-heavy signal rows in the narrow context column now wrap as readable cards instead of vertical stacks. The daily-brief page now renders as a report sheet with real source/status metadata, a two-column desktop brief body, and a single-column mobile report body; history is kept as a compact database record section. The page is still truthful: future AI brief generation is labeled as a backend draft source only, and no fake brief/news content is invented.
- Tests: `git diff --check -- apps/web/app/plans/page.tsx apps/web/app/briefs/page.tsx apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS. Authenticated local production Playwright QA on `127.0.0.1:3106` with `NEXT_PUBLIC_API_BASE_URL=https://api.eycvector.com` swept `/`, `/plans`, `/briefs`, and `/companies/1101` at 1365px desktop and 390px mobile: 8/8 returned 200 with 0 horizontal overflow, 0 narrow vertical text stacks, 0 targeted raw diagnostic hits, 0 console errors, and 0 page errors. Focused reruns also verified `/briefs` mobile uses one report column and `/plans` desktop right-column signals no longer stack vertically. Evidence remains local at `evidence/w7_paper_sprint/local_visual_qa_pass76_plans_briefs_2026-05-03/`.
- Blockers: production post-merge visual smoke is required after deploy. KGI `libCGCrypt.so` remains the live-submit blocker only. Stop-lines respected; QA credentials were used only in process memory and not written to repo/evidence.

### Codex cycle (2026-05-03 20:42 Taipei) - dashboard/order rail final spacing and clean-server QA
- Files changed: updated `apps/web/app/globals.css`, `apps/web/app/companies/[symbol]/OhlcvCandlestickChart.tsx`, `apps/web/app/companies/[symbol]/PaperOrderPanel.tsx`, and this board.
- Endpoints / data behavior: no endpoint contract changed. K-line still uses the existing official OHLCV read path and the simulated-order rail still calls only the existing paper preview/order surfaces. No broker write, live submit, KGI SDK/write-side, Railway secret, migration 0020, RSS/news/commercial-data feature, or destructive DB path was touched. Taiwan unit semantics remain explicit: `SHARE` is odd-lot shares and `LOT` is board lots of 1,000 shares.
- Behavior change: reduced the repeated heavy-card feel by turning shared panels and company/order subpanels into lighter top/bottom rule surfaces, widened dashboard module gutters, compacted the company K-line toolbar and right-side simulated-order ticket, shortened the K-line height while keeping the full official OHLCV range visible, and added robust wrapping to company long notes so company master text cannot widen the page. The right order rail keeps clear odd-lot/board-lot wording and does not expose live broker submit.
- Tests: `git diff --check -- apps/web/app/globals.css apps/web/app/companies/[symbol]/PaperOrderPanel.tsx apps/web/app/companies/[symbol]/OhlcvCandlestickChart.tsx` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS. Clean local production server was restarted after build to avoid stale CSS chunks, then authenticated Playwright QA on `127.0.0.1:3110` swept `/`, `/companies/1101`, `/portfolio`, `/plans`, `/briefs`, and `/themes` at 1365px desktop and 390px mobile: 12/12 returned 200 with 0 horizontal overflow, 0 body overflow, 0 narrow vertical text stacks, 0 raw diagnostic hits, 0 CSS 400/404, 0 console errors, and 0 page errors. Evidence remains local at `evidence/w7_paper_sprint/local_visual_qa_pass79_dashboard_order_2026-05-03/summary-final-pass79.json`.
- Blockers: production post-merge visual smoke is required after deploy. The freeze directive file `evidence/w7_paper_sprint/TO_CODEX_freeze_breach_revert_and_banner_2026-05-02.md` is absent in this worktree, so this cycle followed the latest user-provided freeze/stop-line instructions directly. KGI `libCGCrypt.so` remains the live-submit blocker only. Stop-lines respected; QA credentials were used only in process memory and not written to repo/evidence.

### Codex cycle (2026-05-03 20:58 Taipei) - HUD specificity and oversized-panel correction
- Files changed: updated `apps/web/app/globals.css` and this board.
- Endpoints / data behavior: no endpoint contract changed. This is a CSS specificity/density fix only. No broker write, live submit, KGI SDK/write-side, Railway secret, migration 0020, RSS/news/commercial-data feature, or destructive DB path was touched.
- Behavior change: production smoke after PR #127 confirmed the new CSS was deployed, but also revealed `.panel.hud-frame` still overrode page-specific K-line/company/order padding in some places. This pass lowers the global HUD gutter and raises specificity for `.panel.hud-frame.kline-panel`, `.panel.hud-frame.paper-order-panel`, and `.panel.hud-frame.company-info-panel`, so company detail, K-line, and simulated-order surfaces no longer inherit the old 70px side padding.
- Tests: `git diff --check -- apps/web/app/globals.css` PASS; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS. Focused authenticated local production QA on `127.0.0.1:3110` swept `/companies/1101`, `/`, and `/plans`: all returned 200 with 0 horizontal overflow, 0 body overflow, 0 narrow vertical text stacks, 0 console errors, and 0 page errors. Company detail measured K-line padding `18px 26px 22px`, company info padding `18px 28px 22px`, paper order padding `18px 26px 22px`, and all three surfaces have `border-left: 0px`. Evidence remains local at `evidence/w7_paper_sprint/local_visual_qa_pass80_hud_specificity_2026-05-03/summary.json`.
- Blockers: production post-merge smoke required after deploy. KGI `libCGCrypt.so` remains the live-submit blocker only. Stop-lines respected; QA credentials were used only in process memory and not written to repo/evidence.

### Codex cycle (2026-05-03 22:38 Taipei) - FinMind Sponsor sprint mapping + company K-line context
- Files changed: updated `apps/web/app/companies/[symbol]/CompanyHeroBar.tsx`, `apps/web/app/companies/[symbol]/OhlcvCandlestickChart.tsx`, `apps/web/app/companies/[symbol]/PaperOrderPanel.tsx`, `apps/web/app/globals.css`; added `evidence/w7_paper_sprint/codex_finmind_sponsor_999_frontend_data_sprint_2026-05-03.md`; updated this board.
- Endpoints / data behavior: no endpoint contract changed. This cycle reads the official FinMind GitHub/docs/DataList and the internal Sponsor 999 brief, then turns them into an implementation map. No broker write, live submit, KGI SDK/write-side, Railway secret, migration 0020, RSS/news/commercial-data feature, or destructive DB path was touched. Railway deploy note: the visible red `web / 98cada0f` build failure is a same-commit failed deployment record; Railway also has an active successful `web` deployment for `4111d40`, so the red record is not treated as the current production HEAD by itself.
- Behavior change: company K-line now shows a compact quote/context strip above the chart: latest close, daily change, visible-range high/low, latest volume, official row count, and date context. The company hero quote timestamp now includes month/day and time in Asia/Taipei. The company-side order ticket title is softened to `委託票（模擬）`, wording now says it creates simulated orders only, and the form grid can use two compact columns on wider side rails without changing Taiwan odd-lot/board-lot semantics.
- FinMind product direction: Sponsor 999 is now treated as data fuel for company panels, dashboard source health, daily-brief input, and Quant Lab source-intake status. It is not broker readiness, not strategy acceptance, and not live-order approval. Immediate next API asks are authenticated FinMind status diagnostics and typed `TaiwanStockKBar` adapter; immediate web asks are source truth labels and company data panels for OHLCV, revenue, institutional, margin/short, dividend, and financial summary.
- Tests: `git diff --check -- apps/web/app/companies/[symbol]/CompanyHeroBar.tsx apps/web/app/companies/[symbol]/OhlcvCandlestickChart.tsx apps/web/app/companies/[symbol]/PaperOrderPanel.tsx apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Blockers: production visual smoke required after merge/deploy. KGI `libCGCrypt.so` remains the live-submit blocker only. KBar is verified by Sponsor smoke but still needs Trading Room API integration. `/api/v1/lab/bundles` remains a backend gap before Quant Lab can show real performance bundles. Stop-lines respected.

### Codex cycle (2026-05-03 22:56 Taipei) - FinMind read-only API status + KBar adapter
- Files changed: updated `apps/api/src/data-sources/finmind-client.ts`, `apps/api/src/data-sources/finmind-client.test.ts`, `apps/api/src/server.ts`, and `evidence/w7_paper_sprint/codex_finmind_sponsor_999_frontend_data_sprint_2026-05-03.md`; updated this board.
- Endpoints / data behavior: added authenticated read-only `GET /api/v1/data-sources/finmind/status`. It returns token presence, dataset readiness/blockers, token-safe quota placeholder, source notes, and `updatedAt`; it never returns the token. Added `FinMindClient.getStockKBar(stockId, date)` for official one-day `TaiwanStockKBar` sponsor rows. No broker write, live submit, KGI SDK/write-side, Railway secret, migration 0020, RSS/news/commercial-data feature, or destructive DB path was touched.
- Behavior change: backend now has a safe source-diagnostics contract for the frontend to show FinMind readiness instead of guessing. KBar is now a typed adapter method ready for a future chart/API surface, but it is not wired into trading, broker submit, or unapproved strategy claims.
- Tests: `node --import tsx --test apps/api/src/data-sources/finmind-client.test.ts` PASS, 11/11. `pnpm.cmd --filter @iuf-trading-room/api typecheck` is not the correct isolated command in this worktree before package builds and fails on unresolved workspace packages; repo-native `pnpm.cmd run build:api` PASS, building contracts/db/domain/integrations/api successfully.
- Blockers: frontend still needs to consume `/api/v1/data-sources/finmind/status`. KBar endpoint/page integration is not yet exposed to users. KGI `libCGCrypt.so` remains the live-submit blocker only. `/api/v1/lab/bundles` remains a backend gap before Quant Lab can show real performance bundles. Stop-lines respected.

### Codex cycle (2026-05-03 22:58 Taipei) - dashboard consumes FinMind source status
- Files changed: updated `apps/web/lib/api.ts`, `apps/web/app/page.tsx`, `apps/web/app/globals.css`, and this board.
- Endpoints / data behavior: dashboard now reads the authenticated read-only `GET /api/v1/data-sources/finmind/status` endpoint added in the previous cycle. It shows FinMind token presence, dataset readiness count, pending dataset count, and the first readiness chips without exposing the token. No broker write, live submit, KGI SDK/write-side, Railway secret, migration 0020, RSS/news/commercial-data feature, or destructive DB path was touched.
- Behavior change: 戰情台 source rail and secondary dashboard now include a dedicated `FinMind 資料源` panel, so Sponsor 999 readiness is visible as real source truth instead of hidden in evidence docs. The panel is compact, Traditional Chinese, and uses tight chips with wrapping so it does not recreate the oversized boxed-table problem.
- Tests: `Select-String` mojibake sweep over `apps/web/app/page.tsx` and `apps/web/lib/api.ts` PASS; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Blockers: production post-merge visual smoke required after deploy. KBar HTTP/user-facing integration is still next. KGI `libCGCrypt.so` remains the live-submit blocker only. `/api/v1/lab/bundles` remains a backend gap before Quant Lab can show real performance bundles. Stop-lines respected.

### Codex cycle (2026-05-03 23:05 Taipei) - company KBar read-only route contract
- Files changed: updated `apps/api/src/server.ts`, `apps/web/lib/api.ts`, `evidence/w7_paper_sprint/codex_finmind_sponsor_999_frontend_data_sprint_2026-05-03.md`, and this board.
- Endpoints / data behavior: added authenticated read-only `GET /api/v1/companies/:id/kbar?date=YYYY-MM-DD`. The route resolves UUID or ticker, uses FinMind Sponsor `TaiwanStockKBar`, and returns `{ source, state, reason, stockId, date, rows, updatedAt }` with `LIVE`, `EMPTY`, or `BLOCKED`; token missing is `BLOCKED`, not fake data. Web API now has typed `getCompanyKBar()`. No broker write, live submit, KGI SDK/write-side, Railway secret, migration 0020, RSS/news/commercial-data feature, or destructive DB path was touched.
- Behavior change: KBar is no longer only an internal adapter; it is now a safe API contract ready for the next company chart UI pass. It still does not change order behavior and does not imply broker readiness.
- Tests: `node --import tsx --test apps/api/src/data-sources/finmind-client.test.ts` PASS, 11/11; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd run build:api` PASS.
- Blockers: company chart UI still needs to consume this KBar route. Production post-merge visual/API smoke required after deploy. KGI `libCGCrypt.so` remains the live-submit blocker only. `/api/v1/lab/bundles` remains a backend gap before Quant Lab can show real performance bundles. Stop-lines respected.

### Codex cycle (2026-05-03 23:14 Taipei) - company chart consumes FinMind KBar
- Files changed: updated `apps/web/app/companies/[symbol]/page.tsx`, `apps/web/app/companies/[symbol]/OhlcvCandlestickChart.tsx`, `evidence/w7_paper_sprint/codex_finmind_sponsor_999_frontend_data_sprint_2026-05-03.md`, and this board.
- Endpoints / data behavior: company detail now calls the read-only `getCompanyKBar(company.id, latestDailyBarDate)` client wrapper. Daily/weekly/monthly chart modes still use official OHLCV; `1分 / 5分 / 15分 / 60分` modes use FinMind Sponsor KBar with 5/15/60-minute aggregation in the browser. If the route is unavailable, token is missing, or the date has no rows, the component shows `BLOCKED` or `EMPTY` with a reason. No broker write, live submit, KGI SDK/write-side, Railway secret, migration 0020, RSS/news/commercial-data feature, or destructive DB path was touched.
- Behavior change: the company K-line is no longer limited to only日K/週K/月K. The compact toolbar now includes intraday modes without the old giant selector blocks; the分K status line shows the actual FinMind row count when available and stays truthful when unavailable.
- Tests: `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Blockers: production post-merge visual/API smoke required after deploy because the new web call depends on the new API route landing with it. KGI `libCGCrypt.so` remains the live-submit blocker only. `/api/v1/lab/bundles` remains a backend gap before Quant Lab can show real performance bundles. Stop-lines respected.

### Codex cycle (2026-05-03 23:46 Taipei) - company source truth and empty-validation cleanup
- Files changed: updated `apps/web/app/companies/[symbol]/CompanyInfoPanel.tsx`, `apps/web/app/companies/[symbol]/SourceStatusCard.tsx`, `apps/web/app/companies/[symbol]/page.tsx`, `apps/web/app/globals.css`, and this board.
- Endpoints / data behavior: no new endpoint contract changed. Company detail now passes the read-only FinMind KBar state into the source-status card, so the user can see whether intraday K data is LIVE, EMPTY, or BLOCKED. No broker write, live submit, KGI SDK/write-side, Railway secret, migration 0020, RSS/news/commercial-data feature, or destructive DB path was touched.
- Behavior change: source status no longer shows a meaningless queue count; it now shows a real Taipei timestamp and source detail. Company master validation no longer renders three oversized empty pills when all validation fields are missing; it shows one concise "尚未接資料" explanation and waits for FinMind/strategy verification before drawing conclusions. Source rows and the source panel header were tightened to reduce the heavy boxed look.
...
### Codex cycle (2026-05-04 00:11 Taipei) - FinMind market value panel wiring
- Production deploy status: PR #133 merge commit `201e4cb` deployed successfully to Railway web deployment `d33eeddf-1ce4-4f74-99fb-9353fa025e83` and API deployment `947fe8d7-782c-4936-9c0a-6704895bd13d`.
- Files changed this cycle: `apps/api/src/data-sources/finmind-client.ts`, `apps/api/src/server.ts`, `apps/web/lib/api.ts`, `apps/web/app/companies/[symbol]/FinancialsPanel.tsx`, and this board.
- Endpoints / data behavior: added authenticated read-only `GET /api/v1/companies/:id/market-value?days=365`, backed by FinMind official `TaiwanStockMarketValue` rows (`date`, `stock_id`, `market_value`). `TaiwanStockMarketValue` now reports READY in the FinMind source status list when the token is present. Also made the existing chips endpoint accept both FinMind margin balance field variants (`MarginPurchaseTodayBalance` / `MarginPurchaseToday`, `ShortSaleTodayBalance` / `ShortSaleToday`) so true margin data does not disappear because of schema-name drift.
- UI behavior: company detail `財報與估值` panel now includes a `市值` tab that shows latest and historical market value in 億/兆 units. The panel keeps LIVE / EMPTY / BLOCKED truthfulness and does not fabricate values when FinMind returns no rows.
- Tests: `pnpm.cmd --filter @iuf-trading-room/api typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/api build` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Blockers: no live submit, no KGI SDK/write-side, no Railway secret, no migration 0020, no RSS/news/commercial-data feature, and no destructive DB action touched. Next bypass task: PR/CI/deploy this read-only market-value slice, then continue with FinMind shareholding/holding-shares and company-page layout polish.

### Codex cycle (2026-05-04 00:49 Taipei) - company trading workbench density repair
- Production status: latest deployed app/API entry points are responding: `https://app.eycvector.com/login` 200, `https://app.eycvector.com/companies/2330` 200, and `https://api.eycvector.com/health` 200. GitHub CI and Railway deploy workflow for `0947f83` are green.
- Files changed: updated `apps/web/app/companies/[symbol]/page.tsx`, `apps/web/app/companies/[symbol]/PaperOrderPanel.tsx`, `apps/web/app/globals.css`, and this board.
- Endpoints / data behavior: no endpoint contract changed. No broker write, live submit, KGI SDK/write-side, Railway secret, migration 0020, RSS/news/commercial-data feature, or destructive DB path was touched. Company simulated order still uses only paper preview/order APIs.
- Behavior change: moved the company KPI strip directly below the hero so the chart workbench opens with source/momentum context instead of hiding that state below the fold. The company-side simulated order ticket now receives the latest official K-line price as its default limit reference, keeps zero-share default semantics (`零股` = shares), keeps board-lot semantics explicit (`整張` = 1,000 shares), and uses a lighter rail instead of heavy boxed controls. K-line and FinMind tab controls were tightened so they read as compact tool controls rather than large template blocks.
- Tests: `git diff --check -- apps/web/app/companies/[symbol]/page.tsx apps/web/app/companies/[symbol]/PaperOrderPanel.tsx apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Blockers / bypass: local `next start` visual harness on port 3112 returns HTTP 500 even though production routes and build are healthy; no credentials were written to command output/evidence/repo. Next bypass task is either production browser smoke with an existing signed-in session or continue static/build-safe UI repairs on dashboard/company panels while preserving 4-state truthfulness.

### Codex cycle (2026-05-04 01:03 Taipei) - dashboard Taiwan data command deck
- Files changed: updated `apps/web/app/page.tsx`, `apps/web/app/globals.css`, and this board.
- Endpoints / data behavior: no endpoint contract changed. Dashboard now uses existing authenticated read-only surfaces only: market overview, companies, strategy ideas, company announcements, watchlist, and FinMind source status. No broker write, live submit, KGI SDK/write-side, Railway secret, migration 0020, RSS/news/commercial-data feature, or destructive DB path was touched.
- Behavior change: added a new `資料接線圖` band under the market tape so the homepage reads like a Taiwan-stock data command center instead of repeated boxed panels. It shows real LIVE / EMPTY / BLOCKED state for行情核心, 公司資料, 策略候選, 重大訊息, and FinMind Sponsor, plus compact Sponsor dataset chips. The hero copy now says clearly that KGI live submit is still locked and the page is for real data/source readiness first.
- Tests: `git diff --check -- apps/web/app/page.tsx apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Blockers / bypass: production deploy and post-deploy smoke are next. The freeze directive file `evidence/w7_paper_sprint/TO_CODEX_freeze_breach_revert_and_banner_2026-05-02.md` is still absent in this worktree, so this cycle followed the latest user-provided freeze/stop-line instructions directly. KGI `libCGCrypt.so` remains the only live-submit blocker.

### Codex cycle (2026-05-04 01:24 Taipei) - company K-line backfill and workbench polish
- Files changed: updated `apps/api/src/companies-ohlcv.ts`, `apps/web/app/companies/[symbol]/OhlcvCandlestickChart.tsx`, `apps/web/app/globals.css`, and this board.
- Endpoints / data behavior: `GET /api/v1/companies/:id/ohlcv` remains read-only, but daily Taiwan OHLCV now uses FinMind as a backfill when DB daily bars are too thin (<220 rows) and the authenticated ticker/token path is available. If FinMind fails, the endpoint keeps the existing DB rows instead of crashing. No broker write, live submit, KGI SDK/write-side, Railway secret, migration 0020, RSS/news/commercial-data feature, or destructive DB path was touched.
- Behavior change: company charts can show a materially longer real daily K history instead of stopping at a short DB cache slice. The K-line chart is taller, and company detail proportions now prioritize the chart while keeping the simulated-order rail as a supporting panel. Taiwan unit safety remains unchanged: 零股 is shares; 整張 is 1,000 shares.
- Tests: `git diff --check -- apps/api/src/companies-ohlcv.ts apps/web/app/companies/[symbol]/OhlcvCandlestickChart.tsx apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/api typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/api build` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Blockers / bypass: production PR/CI/deploy and post-deploy smoke are next. KGI `libCGCrypt.so` remains the only live-submit blocker.

### Codex cycle (2026-05-04 01:35 Taipei) - dashboard/theme/brief copy repair
- Files changed: updated `apps/web/app/page.tsx`, `apps/web/app/themes/page.tsx`, `apps/web/app/briefs/page.tsx`, `apps/web/lib/operator-copy.ts`, and this board.
- Endpoints / data behavior: no endpoint contract changed. This is a frontend truthfulness/copy repair only; it keeps all reads on existing authenticated routes and does not touch broker write, live submit, KGI SDK/write-side, Railway secrets, migration 0020, RSS/news/commercial-data, or destructive DB paths.
- Behavior change: reverted corrupted dashboard command-deck copy back to clean Traditional Chinese and restored clean Traditional Chinese copy for the main theme board and daily brief page. Rebuilt `operator-copy.ts` with clean fallback/mapping text so English-heavy headlines or source terms no longer turn into mojibake. The goal is to stop corrupted text from reaching production while preserving the 4-state LIVE / EMPTY / BLOCKED behavior. FinMind Sponsor remains available through the already-merged status/API slices; the next clean pass will re-add any source-readiness presentation in readable Traditional Chinese only.
- Deploy status: previous PR #138 / merge commit `c6ca6d8` deployed successfully to Railway for worker, web, and api. The K-line backfill is now live; this cycle is the follow-up visual/text repair.
- Tests: `git diff --check -- apps/web/app/page.tsx apps/web/app/themes/page.tsx apps/web/app/briefs/page.tsx apps/web/lib/operator-copy.ts evidence/w7_paper_sprint/frontend_realdata_status_board_2026-05-01.md` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS. Production route smoke after #138 deploy: `/login` 200, `/companies/2330` 200, API `/health` 200.
- Blockers / bypass: production visual smoke still needs an authenticated session, but this branch can be verified by build/typecheck and route smoke first. KGI `libCGCrypt.so` remains the only live-submit blocker.

### Codex cycle (2026-05-04 09:40 Taipei) - dashboard clean FinMind command deck recovery
- Files changed: updated `apps/web/app/page.tsx` and this board.
- Endpoints / data behavior: no endpoint contract changed. The dashboard reads only existing authenticated read-only surfaces: market overview, companies, themes, strategy ideas, strategy runs, signals, announcements, watchlist, and FinMind source status. No broker write, live submit, KGI SDK/write-side, Railway secret, migration 0020, RSS/news/commercial-data feature, or destructive DB path was touched.
- Behavior change: rewrote the dashboard command deck copy and source rail in clean Traditional Chinese. The homepage now opens as a Taiwan-stock command center with a market tape, real source-state chips, and a compact FinMind Sponsor readiness band. It explicitly states that KGI live submit remains locked behind `libCGCrypt.so`; no UI state implies live trading is available.
- Tests: `git diff --check -- apps/web/app/page.tsx` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS; UTF-8 scan over `apps/web/app/page.tsx` found no U+FFFD/private-use mojibake characters.
- Blockers / bypass: production PR/CI/deploy and post-deploy smoke are next. The freeze directive file is absent in this demo worktree, but the copy in the main worktree confirms the same stop-lines; this cycle stayed inside safe frontend/evidence work. KGI `libCGCrypt.so` remains the only live-submit blocker.

### Codex cycle (2026-05-04 10:34 Taipei) - daily brief report surface recovery
- Files changed: updated `apps/web/app/briefs/page.tsx` and this board.
- Endpoints / data behavior: no endpoint contract changed. `/briefs` still reads the existing authenticated daily-brief database endpoint and does not invent AI/news content. No broker write, live submit, KGI SDK/write-side, Railway secret, migration 0020, RSS/news/commercial-data feature, or destructive DB path was touched.
- Behavior change: the latest daily brief now renders as a distinct report sheet with source, market-state, date, and update metadata in clean Traditional Chinese instead of using the same generic boxed panel template as every other page. The history block is kept as a compact database record section. This keeps the page ready for a future AI-generated Taiwan-stock daily report while remaining truthful today.
- Tests: `git diff --check -- apps/web/app/briefs/page.tsx` PASS; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Blockers / bypass: production PR/CI/deploy and post-deploy smoke are next. KGI `libCGCrypt.so` remains the only live-submit blocker.

### Codex cycle (2026-05-04 11:06 Taipei) - company K-line/order workbench polish
- Files changed: updated `apps/web/app/companies/[symbol]/OhlcvCandlestickChart.tsx`, `apps/web/app/companies/[symbol]/PaperOrderPanel.tsx`, `apps/web/app/companies/[symbol]/CompanyHeroBar.tsx`, `apps/web/app/companies/[symbol]/SourceStatusCard.tsx`, `apps/web/app/globals.css`, and this board.
- Endpoints / data behavior: no endpoint contract changed. Company detail still reads existing company master, official OHLCV, FinMind KBar, paper-preview/order, and source-status surfaces. No broker write, live submit, KGI SDK/write-side, Railway secret, migration 0020, RSS/news/commercial-data feature, or destructive DB path was touched.
- Behavior change: the company K-line now has an in-chart latest-price ribbon so the chart is not visually detached from price. The company main/side grid gives the chart more usable width, while the simulated-order rail is tighter and less like stacked hard boxes. The order rail source, lock-note, latest-price, and form sections now use reusable CSS classes with clearer breathing room. Taiwan unit safety remains unchanged: `零股` means shares and `整張` means board lots of 1,000 shares. Company-side timestamp formatting is now deterministic Taipei time to prevent React hydration text mismatches.
- Tests: `git diff --check -- apps/web/app/companies/[symbol]/PaperOrderPanel.tsx apps/web/app/companies/[symbol]/OhlcvCandlestickChart.tsx apps/web/app/companies/[symbol]/CompanyHeroBar.tsx apps/web/app/companies/[symbol]/SourceStatusCard.tsx apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web build` PASS; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS after build regenerated `.next/types`. Authenticated local production Playwright QA on `127.0.0.1:3120/companies/2330` at 1365px returned 200 with 0 console errors, 0 page errors, 0 horizontal overflow, 0 narrow vertical text stacks, visible latest-price ribbon (`最新收盤 2,135 2026-04-30`), and a compact odd-lot default order grid. Evidence remains local at `evidence/w7_paper_sprint/local_visual_qa_pass82_company_order_2026-05-04_rerun/`.
- Blockers / bypass: production PR/CI/deploy and post-deploy smoke are next. KGI `libCGCrypt.so` remains the only live-submit blocker; all live broker paths remain untouched. QA credentials were used only in process memory for the transient Playwright session and were not written to repo/evidence.

### Codex cycle (2026-05-04 11:45 Taipei) - compact label and border breathing repair
- Files changed: updated `apps/web/app/globals.css` and this board.
- Endpoints / data behavior: no endpoint contract changed. This is a CSS-only repair for readability and does not touch broker write, live submit, KGI SDK/write-side, Railway secrets, migration 0020, RSS/news/commercial-data, or destructive DB paths.
- Behavior change: short status labels and compact controls (`正常`, `無資料`, `暫停`, quote values, buttons, K-line tabs, source badges) now force horizontal writing with safer wrapping, so narrow panels cannot split them into ugly vertical fragments. Panel headers also have slightly more padding and bottom spacing so labels/icons are not pressed directly against separator lines. Paper-order helper text keeps Chinese words together while still allowing long helper copy to wrap safely.
- Tests: `git diff --check -- apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- QA / bypass: production route sweep screenshots were captured under `evidence/w7_paper_sprint/production_smoke_pass83_route_sweep_2026-05-04/`. Local post-patch authed visual QA is blocked by normal domain-cookie behavior because `localhost` cannot receive the production `.eycvector.com` session cookie; the bypass is to deploy this CSS-only patch and verify on production with the real app domain.
- Blockers: none for this PR. KGI `libCGCrypt.so` remains the only live-submit blocker; Taiwan unit safety remains unchanged (`零股` = shares, `整張` = 1,000 shares).

### Codex cycle (2026-05-04 12:02 Taipei) - production compact-label verification and trading-room rhythm pass
- Production deploy status: PR #143 / merge commit `4616287` deployed successfully to Railway for api, worker, and web.
- Production QA: authenticated 1365px route sweep on `/`, `/companies`, `/themes`, `/plans`, `/portfolio`, `/briefs`, and `/companies/2330` returned 0 console errors, 0 page errors, 0 horizontal overflow, and 0 compact-label vertical stacks. Evidence screenshots are under `evidence/w7_paper_sprint/production_smoke_pass83_compact_labels_postdeploy_2026-05-04/`.
- Files changed after verification: started the next safe frontend rhythm pass in `apps/web/app/portfolio/page.tsx` and `apps/web/app/globals.css`.
- Behavior change in progress: trading-room layout now separates account summary, risk overview, order workbench, and ledger blocks into route-specific rhythm classes instead of reusing the old generic `exec-grid` feel. No order logic, unit conversion, broker write, live submit, KGI SDK/write-side, Railway secrets, migration 0020, RSS/news/commercial-data, or destructive DB paths were touched.
- Tests: `git diff --check -- apps/web/app/portfolio/page.tsx apps/web/app/globals.css evidence/w7_paper_sprint/frontend_realdata_status_board_2026-05-01.md` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web build` PASS; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS after build regenerated `.next/types`. Local 1365px layout QA on `/portfolio` with a fake middleware-only session returned 0 console errors, 0 page errors, 0 horizontal overflow, and 0 compact-label vertical stacks; this local pass intentionally did not forge API auth, so data panels showed truthful BLOCKED state while checking layout rhythm.
- Blockers: KGI `libCGCrypt.so` remains the only live-submit blocker. Taiwan unit safety remains unchanged (`零股` = shares, `整張` = 1,000 shares).

### Codex cycle (2026-05-04 16:18 Taipei) - company financial pagination and FinMind KBar fallback
- Files changed: updated `apps/web/app/companies/[symbol]/FinancialsPanel.tsx`, `apps/web/app/companies/[symbol]/OhlcvCandlestickChart.tsx`, `apps/web/app/globals.css`, `apps/web/lib/api.ts`, `apps/api/src/server.ts`, `apps/web/app/portfolio/page.tsx`, and this board.
- Endpoints / data behavior: `GET /api/v1/companies/:id/kbar` stays authenticated and read-only, but now falls back across recent calendar dates when the requested date has no FinMind Sponsor `TaiwanStockKBar` rows. This prevents a valid Sponsor token from showing a dead minute chart just because the selected latest daily bar date is stale or a holiday. No broker write, live submit, KGI SDK/write-side, Railway secrets, migration 0020, RSS/news/commercial-data, or destructive DB path was touched.
- UI behavior: company K-line copy was rebuilt in clean Traditional Chinese; 日K / 週K / 月K / 1分 / 5分 / 15分 / 60分 controls remain compact and truthful. Company 財報與估值 panel was rebuilt in clean Traditional Chinese, status badges now use green for 正常 and red for 無法顯示, and long 財報 / 月營收 / 估值 / 市值 / 股利 tables are capped to 10 visible rows with 上一頁 / 下一頁 controls instead of stretching the page into a giant block.
- Tests: `git diff --check` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/api typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS; `pnpm.cmd --filter @iuf-trading-room/api build` PASS. Production API spot check before deploy confirmed existing `2330` FinMind KBar returns `LIVE` with 266 rows and market-value returns rows.
- Local QA: 1365px authenticated local SSR smoke on `/companies/2330` returned 0 console errors, 0 page errors, 0 horizontal overflow, 0 compact-label vertical stacks, no mojibake, clean K-line buttons, and a working 1分 click with FinMind Sponsor minute data rendered. Local browser-to-production API calls for client-only financial tabs are blocked by normal localhost/CORS cookie scope, so final financial-tab visual verification must happen on production after deploy.
- Blockers / bypass: next step is PR/CI/Railway deploy, then production authenticated QA for `/companies/2330` focusing on financial table pagination, status colors, and 1/5/15/60 分 K interactions. KGI `libCGCrypt.so` remains the only live-submit blocker. Taiwan unit safety remains unchanged: `零股` means shares, `整張` means 1,000 shares.

## Codex Cycle (2026-05-04 21:05 Taipei) - company workbench width and minute K unblock
- Files changed: updated `apps/web/app/companies/[symbol]/page.tsx`, `apps/web/app/companies/[symbol]/OhlcvCandlestickChart.tsx`, `apps/web/app/companies/[symbol]/PaperOrderPanel.tsx`, `apps/web/app/globals.css`, and this board.
- Behavior: company detail now keeps the K 線 and 模擬委託 in the main working column. At common desktop widths below 1500px the company detail layout stacks to one column, so the order ticket no longer gets squeezed into a narrow right rail. The right rail now carries company/source context below the main workbench.
- Minute K: intraday 1 / 5 / 15 / 60 分 charts no longer reuse the daily-chart 12-bar trend gate. Sparse but real 60 分 data now renders truthful candlesticks instead of showing a misleading 資料不足 block.
- Status colors: the order ledger `正常` state now uses green; `暫停/無法顯示` remains red. `無資料` stays amber as truthful EMPTY.
- Tests: `git diff --check` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Local QA: authenticated 1365px local production-server smoke on `/companies/2330` returned 0 horizontal overflow, 0 mojibake/replacement characters, 0 narrow stacked text candidates, 0 console/page errors. K-line clicks verified 1分=266 bars, 5分=54 bars, 15分=19 bars, 60分=5 bars; none showed 無資料, 無法顯示, or 資料不足.
- Stop-lines: no live submit, no KGI SDK/write-side, no Railway secrets, no migration 0020, no destructive DB action, no RSS/news/commercial-data feature was touched.


## Codex Cycle (2026-05-04 21:58 Taipei) - global status color semantics repair
- Files changed: updated status/data-tone usage in apps/web/app/page.tsx, companies/page.tsx, ideas/page.tsx, plans/page.tsx, portfolio/page.tsx, runs/page.tsx, runs/[id]/page.tsx, signals/page.tsx, themes/page.tsx, themes/[short]/page.tsx, market-intel/page.tsx, ops/page.tsx, m/page.tsx, m/kill/page.tsx, lab/LabClient.tsx, lab/[bundleId]/LabBundleDetailClient.tsx, apps/web/components/RadarWidgets.tsx, and this board.
- Behavior: separated data/governance status color from Taiwan stock price movement. LIVE/normal/pass/healthy/usable/ready/approved states now use status-ok green; paused/blocked/failed/rejected/danger/unavailable states now use status-bad red. Stock movement and signal direction still keep the Taiwan convention: price/return up stays red, down stays green.
- Data / endpoint behavior: no endpoint contract changed. This is frontend semantics and visual truthfulness only. No live submit, KGI SDK/write-side, Railway secret, migration 0020, destructive DB action, or RSS/news/commercial-data feature was touched.
...
## Codex Cycle (2026-05-04 23:53 Taipei) - layout rhythm and order-ticket workbench
- Files changed: updated apps/web/app/companies/[symbol]/PaperOrderPanel.tsx, apps/web/app/globals.css, and this board.
- Behavior: rebuilt the company-page simulated order station into a two-zone workbench: left side is the compact order ticket and right side is risk preview plus per-symbol simulated-order ledger. This removes the long stacked block feeling while preserving the explicit Taiwan unit guard: SHARE means odd-lot shares, LOT means board lots of 1,000 shares. Segment controls and inputs were tightened so the ticket reads like a tool, not a row of large bricks. Dashboard mosaic CSS was also tuned so the primary and secondary lanes have different weight and breathing room instead of every panel feeling identical.
- Data / endpoint behavior: no endpoint contract changed. No live submit, no KGI SDK/write-side, no Railway secret, no migration 0020, no destructive DB action, and no RSS/news/commercial-data feature was touched.
- Tests: pnpm.cmd --filter @iuf-trading-room/web typecheck PASS; pnpm.cmd --filter @iuf-trading-room/web build PASS; git diff --check PASS with CRLF warnings only.
- Local QA: 1365px local production-server smoke using a temporary read-only fake API for company 2330 returned /companies/2330 status 200, 0 console errors, 0 page errors, 0 horizontal overflow, 0 replacement characters, 0 narrow vertical text stacks, visible K-line canvas, and paper-order workbench columns 420px / 444.922px. Evidence: evidence/w7_paper_sprint/local_visual_qa_pass89_layout_rhythm_fakeapi_2026-05-04/. Dashboard blocked-state visual smoke at 1365px also returned 0 overflow and 0 narrow stacks. The temporary fake API was local only and returned read-only fixture data; it did not touch the production API or database.
- Blockers / bypass: production PR/CI/deploy and production-domain post-deploy smoke are next. KGI libCGCrypt.so remains the only live-submit blocker.

## Codex Cycle (2026-05-05 00:35 Taipei) - company finance panel copy and pagination repair
- Files changed: updated apps/web/app/companies/[symbol]/FinancialsPanel.tsx, apps/web/app/globals.css, and this board.
- Behavior: rebuilt the company 財報與估值 panel copy in clean Traditional Chinese and moved the FinMind dataset selector from large rigid blocks into a compact horizontal tool strip. The active dataset summary now shows source, row count, and Taipei update time without stretching the page. All data tables stay capped to 10 visible rows per page with 上一頁 / 下一頁 controls, so 市值、估值、月營收、股利 and raw balance/cash-flow source rows no longer push the page into one giant column.
- Data / endpoint behavior: no endpoint contract changed. The panel still reads existing authenticated read-only FinMind-backed endpoints only: financials, revenue, balance-sheet, cash-flow, valuation, market-value, and dividend. No live submit, no KGI SDK/write-side, no Railway secret, no migration 0020, no destructive DB action, and no RSS/news/commercial-data feature was touched.
- Tests: pnpm.cmd --filter @iuf-trading-room/web typecheck PASS; pnpm.cmd --filter @iuf-trading-room/web build PASS; git diff --check PASS with CRLF warnings only. A source scan over FinancialsPanel found 0 replacement/private-use bad characters.
- Local QA: 1365px local production-server smoke against a temporary read-only fake API rendered /companies/2330 with the finance panel present, 0 console/page errors, 0 horizontal overflow, 0 replacement characters, and 0 narrow vertical text stacks. The fixture server is local-only and does not touch production API or DB. Client-side live-data click verification still needs production-domain smoke after deploy because NEXT_PUBLIC_API_BASE_URL is baked at web build time.
- Blockers / bypass: production PR/CI/deploy and production-domain post-deploy smoke are next. KGI libCGCrypt.so remains the only live-submit blocker; Taiwan unit safety is unchanged.

## Codex Cycle (2026-05-05 01:06 Taipei) - company master card and industry labels
- Files changed: updated `apps/web/app/companies/[symbol]/CompanyInfoPanel.tsx`, `apps/web/lib/industry-i18n.ts`, `apps/web/app/globals.css`, and this board.
- Behavior: rebuilt the company master panel into a compact identity card instead of a raw database dump. The top now shows ticker, company name, market, beneficiary tier, and chain label in one readable block; exposure scores are tighter meter rows; validation fields are explicit source-state pills; long notes are capped in a scrollable note box so the page no longer stretches into a giant slab. Added translations for `semiconductor` / `Semiconductor` / `Foundry` so company headings and notes render as `半導體` / `晶圓代工` instead of raw English slugs.
- Data / endpoint behavior: no endpoint contract changed. This pass only changes read-only company detail rendering and industry label translation. No live submit, KGI SDK/write-side, Railway secret, migration 0020, destructive DB action, or RSS/news/commercial-data feature was touched.
- Tests: `git diff --check -- apps/web/app/companies/[symbol]/CompanyInfoPanel.tsx apps/web/app/globals.css apps/web/lib/industry-i18n.ts` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS; source scan found 0 replacement/private-use bad characters in changed files.
- Local QA: 1365px local production-server smoke on `/companies/2330` with a middleware-only local session and a temporary read-only fake API returned 0 console errors, 0 page errors, 0 horizontal overflow, 0 replacement/private-use characters, and 0 narrow vertical text stacks. Company master text now shows `半導體` and `晶圓代工`; evidence is under `evidence/w7_paper_sprint/local_visual_qa_pass91_company_info_2026-05-05/`.
- Blockers / bypass: production PR/CI/deploy and production-domain post-deploy smoke are next. KGI `libCGCrypt.so` remains the only live-submit blocker; Taiwan unit safety is unchanged (`零股` = actual shares, `整張` = 1,000 shares).

## Codex Cycle (2026-05-05 01:31 Taipei) - company order ticket summary strip
- Files changed: updated `apps/web/app/companies/[symbol]/PaperOrderPanel.tsx`, `apps/web/app/globals.css`, and this board.
- Behavior: rebuilt the company-page simulated-order ticket summary into a compact trading strip instead of stacked helper blocks. The ticket now shows `股票 / 單位 / 實際股數 / 預估金額` in one readable row, with `零股` explained as shares and `整張` preserved as a 1,000-share board-lot path. The risk preview side now has a lighter console surface so the order ticket reads as one workbench instead of disconnected hard boxes.
- Data / endpoint behavior: no endpoint contract changed. The panel still uses existing read-only K-line/company data plus existing paper preview/order endpoints only. No live submit, KGI SDK/write-side, Railway secret, migration 0020, destructive DB action, RSS/news/commercial-data feature, or fake success path was touched.
- Tests: `git diff --check -- apps/web/app/companies/[symbol]/PaperOrderPanel.tsx apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS; changed-file bad-character scan found 0 replacement/private-use characters.
- Local QA: 1365px local production-server smoke on `/companies/2330` with a middleware-only local session and temporary read-only fake API returned status 200, 0 console errors, 0 page errors, 0 horizontal overflow, 0 bad characters, and 0 narrow vertical text stacks. K-line buttons `1分 / 5分 / 15分 / 60分` were clicked successfully and the K-line canvas stayed visible. Evidence is under `evidence/w7_paper_sprint/local_visual_qa_pass92_order_ticket_2026-05-05_rerun/`.
- Deploy note: Railway web deployment for `84dd4f4` was still reporting `BUILDING` while the previous `225fcbd` deployment was `SUCCESS`; next cycle should re-check Railway status before assuming production has this and pass91.
- Blockers / bypass: KGI `libCGCrypt.so` remains the only live-submit blocker. Taiwan unit safety is unchanged and explicit: `零股` = actual shares, `整張` = 1,000 shares.

## Codex Cycle (2026-05-05 01:51 Taipei) - plans and daily-brief rhythm repair
- Files changed: updated `apps/web/app/globals.css` and this board.
- Behavior: repaired the `交易計畫` page rhythm without changing endpoint contracts. The plan ledger now renders as decision work orders instead of a dead table; strategy ideas use a separate candidate-rail treatment; the daily brief reads like a report sheet; review and signal sections use lighter timeline surfaces. At 1365px the plans workbench stacks safely instead of squeezing the right rail into narrow columns.
- Data / endpoint behavior: no endpoint contract changed. The page still reads existing authenticated read-only plans, companies, themes, signals, briefs, reviews, and strategy-ideas endpoints. It does not create AI briefs, does not invent news, and does not expose fake success. No live submit, KGI SDK/write-side, Railway secret, migration 0020, destructive DB action, RSS/news/commercial-data feature, or broker path was touched.
- Tests: `git diff --check -- apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Local QA: local production-server smoke on `/plans` at 1365px returned status 200, 0 console errors, 0 page errors, 0 horizontal overflow, 0 replacement/private-use bad characters, and 0 narrow vertical text stacks. A temporary read-only fixture API on `127.0.0.1:3132` returned two plans, one daily brief, two ideas, one review, and two signals so the LIVE layout was verified instead of only EMPTY/BLOCKED state. Evidence is under `evidence/w7_paper_sprint/local_visual_qa_pass93_plans_rhythm_2026-05-05/`.
- Blockers / bypass: production PR/CI/deploy and post-deploy smoke are next. KGI `libCGCrypt.so` remains the only live-submit blocker; Taiwan unit safety remains unchanged.

## Codex Cycle (2026-05-05 02:04 Taipei) - dashboard command-deck visual recovery
- Files changed: updated `apps/web/app/page.tsx`, `apps/web/app/globals.css`, and this board.
- Behavior: rebuilt the homepage hero away from a cramped two-column card into a wider command-deck surface with a stronger Taiwan-stock headline, source-state rail, market tape, and less boxy KPI treatment. The copy now reads as a trading-room dashboard instead of instructions about mock avoidance, while still explicitly keeping KGI live submit locked behind `libCGCrypt.so`.
- Data / endpoint behavior: no endpoint contract changed. The dashboard still reads existing authenticated read-only market overview, companies, themes, strategy ideas, runs, signals, announcements, watchlist, and FinMind status surfaces. No live submit, KGI SDK/write-side, Railway secret, migration 0020, destructive DB action, RSS/news/commercial-data feature, or broker path was touched.
- Tests: `git diff --check -- apps/web/app/page.tsx apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Local QA: local production-server smoke on `/` at 1365px with a temporary read-only fixture API returned status 200, 0 console errors, 0 page errors, 0 failed responses, 0 horizontal overflow, 0 replacement/private-use bad characters, and 0 narrow vertical text stacks. Evidence is under `evidence/w7_paper_sprint/local_visual_qa_pass94_dashboard_hero_2026-05-05/`.
- Deploy note: Railway web latest visible deployment is still `13ef283`; PR #152 merge commit `a5c344c` has not appeared in Railway deployment list yet at this checkpoint. Re-check before claiming production has pass93/pass94.

## Codex Cycle (2026-05-05 02:28 Taipei) - company K-line and order-desk de-blocking
- Files changed: updated `apps/web/app/companies/[symbol]/OhlcvCandlestickChart.tsx`, `apps/web/app/globals.css`, and this board.
- Behavior: turned the company K-line controls into a compact tool strip with explicit `週期` / `範圍` labels, converted the four large K-line snapshot blocks into a thinner quote rail, and gave the chart more vertical reading room. The company simulated-order desk now reads as one workbench: the ticket is compact, the summary is a small trading strip, and the sidecar stays beside the ticket at 1365px instead of feeling like stacked slabs. The company master/source panels stack as lighter data drawers below the workbench at common desktop widths.
- Data / endpoint behavior: no endpoint contract changed. This pass only changes company detail rendering and does not touch live submit, KGI SDK/write-side, Railway secrets, migration 0020, destructive DB action, RSS/news/commercial-data feature, or broker paths. Taiwan unit safety remains explicit: `零股` = actual shares and `整張` = board lots of 1,000 shares.
- Tests: `git diff --check -- apps/web/app/companies/[symbol]/OhlcvCandlestickChart.tsx apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Local QA: 1365px local production-server smoke on `/companies/2330` with a temporary read-only fixture API returned status 200, 0 console errors, 0 page errors, 0 horizontal overflow, 0 replacement/private-use bad characters, and 0 narrow vertical text stacks. K-line clicks verified `1分 / 5分 / 15分 / 60分` all keep a visible canvas and do not show K-line `無資料 / 無法顯示 / 資料不足`; 60分 rendered 5 real aggregated bars. Evidence is under `evidence/w7_paper_sprint/local_visual_qa_pass95_company_workbench_2026-05-05/`.
- Deploy note: Railway web has `a5c344c` SUCCESS and is currently BUILDING merge commit `32b1513`; this pass is not yet pushed/deployed. Next step is PR/CI/deploy, then production-domain smoke for `/companies/2330`.

## Codex Cycle (2026-05-05 03:05 Taipei) - login/register account gateway repair
- Files changed: updated `apps/web/app/login/page.tsx`, `apps/web/app/register/page.tsx`, `apps/web/app/globals.css`, and this board.
- Behavior: rebuilt the login/register entry copy and layout so it explains the real product path in Traditional Chinese: this is an IUF website account, not a brokerage login; invited users can create a testing account now; brokerage binding, subscription tiers, and AI daily brief permissions are future account capabilities. The old checklist-like feel is replaced by a clearer account capability rail and a stronger form gateway. Status colors follow the repaired semantics: usable states are green, future/reserved states are amber.
- Data / endpoint behavior: no endpoint contract changed. Registration still uses the existing invite-code auth endpoint and login still uses the existing real auth endpoint. No fake account creation, live submit, KGI SDK/write-side, Railway secret, migration 0020, destructive DB action, RSS/news/commercial-data feature, or broker path was touched.
- Tests: `git diff --check -- apps/web/app/login/page.tsx apps/web/app/register/page.tsx apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS; changed-file bad-character scan found 0 replacement/private-use characters.
- Local QA: local production-server smoke on `/login` and `/register` at 1365px and 390px returned status 200, 0 console errors, 0 page errors, 0 failed requests, 0 horizontal overflow, 0 bad characters, and 0 narrow vertical text stacks. Evidence is local at `evidence/w7_paper_sprint/local_visual_qa_pass96_login_register_2026-05-05/`.
- Blockers / bypass: production PR/CI/deploy and production-domain smoke are next. KGI `libCGCrypt.so` remains the only live-submit blocker; Taiwan unit safety is unchanged.

## Codex Cycle (2026-05-05 03:35 Taipei) - market intel official-announcement surface repair
- Files changed: updated `apps/web/app/market-intel/page.tsx`, `apps/web/app/globals.css`, and this board.
- Behavior: rebuilt the `重大訊息` page away from the generic table/panel template into a route-specific official-announcement monitor. It now clearly says the page reads company-linked TWSE OpenAPI 重大訊息, shows full Taipei date stamps, displays tracked tickers as compact chips, and renders EMPTY/BLOCKED truth states without fake news. If strategy ideas return an unexpected shape, the page now fails closed instead of throwing a 500.
- Data / endpoint behavior: no endpoint contract changed. The page still reads existing authenticated read-only company, strategy-idea, and company-announcement endpoints only. No RSS, commercial-news integration, AI-generated news, live submit, KGI SDK/write-side, Railway secret, migration 0020, destructive DB action, or broker path was touched.
- Tests: `git diff --check -- apps/web/app/market-intel/page.tsx apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS; changed-file bad-character scan found 0 replacement/private-use characters.
- Local QA: local production-server smoke on `/market-intel` at 1365px and 390px with a middleware-only session returned status 200, 0 console errors, 0 page errors, 0 horizontal overflow, 0 bad characters, 0 narrow vertical text stacks, and no login redirect. Evidence is local at `evidence/w7_paper_sprint/local_visual_qa_pass97_market_intel_2026-05-05/`. A temporary read-only fixture API was used to validate announcement-shaped LIVE rendering during development, then final build/QA returned to normal no-env fail-closed behavior.
- Deploy note: PR #155 / `5552f7c` was merged before this cycle. Railway web was still active on `d04537c` at the last deployment poll; re-check after this PR.
- Blockers / bypass: real news/AI daily article generation remains deferred until approved source/licensing/backend contract. KGI `libCGCrypt.so` remains the only live-submit blocker.

## Codex Cycle (2026-05-05 03:55 Taipei) - theme and signal surfaces de-table pass
- Production deploy status: Railway web now shows PR #156 / `c6023617` SUCCESS, so the login/register and market-intel repairs are on production.
- Files changed: updated `apps/web/app/themes/page.tsx`, `apps/web/app/signals/page.tsx`, `apps/web/app/globals.css`, and this board.
- Behavior: rebuilt `主題板` from a cramped table into a route-specific theme radar card wall. Theme rows now carry priority, market state, lifecycle, core/observation pool counts, full source timestamp, and cleaned Traditional Chinese thesis copy without overlapping columns or raw orphan labels.
- Behavior: rebuilt `訊號證據` from a table ledger into a chronological signal tape. Signal rows now show full Taipei date+time, category, direction, confidence, linked company/theme chip, and source in a readable timeline surface. Internal dry-run/smoke/test signals are explicitly collected and hidden from the formal signal list instead of mixing with real evidence.
- Data / endpoint behavior: no endpoint contract changed. Both pages still read existing authenticated read-only `themes`, `signals`, and `companies` endpoints only. No RSS/news/commercial-data feature, AI-generated news, live submit, KGI SDK/write-side, Railway secret, migration 0020, destructive DB action, or broker path was touched.
- Tests: `git diff --check -- apps/web/app/themes/page.tsx apps/web/app/signals/page.tsx apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS; changed-file bad-character scan found 0 replacement/private-use characters.
- Local QA: local production-server smoke on `/themes` and `/signals` at 1365px and 390px with a middleware-only session plus temporary read-only fixture API returned status 200, 0 console errors, 0 horizontal overflow, 0 replacement/private-use bad characters, 0 English-heavy blocks, and 0 narrow vertical text stacks. Evidence is local at `evidence/w7_paper_sprint/local_visual_qa_pass98_themes_signals_2026-05-05/`. Playwright recorded only normal Next.js prefetch `ERR_ABORTED` noise while navigating between routes.
- Blockers / bypass: production PR/CI/deploy and post-deploy smoke are next. KGI `libCGCrypt.so` remains the only live-submit blocker; Taiwan unit safety remains unchanged (`零股` = actual shares, `整張` = 1,000 shares).

## Codex Cycle (2026-05-05 03:40 Taipei) - theme detail workbench repair
- Files changed: updated `apps/web/app/themes/[short]/page.tsx`, `apps/web/app/globals.css`, and this board.
- Behavior: rebuilt the individual `主題明細` page from a raw endpoint/source table into a route-specific theme workbench. The page now uses clean Traditional Chinese source labels, lifecycle labels, cleaned theme thesis copy, member-company cards, strategy-idea cards, and a compact signal tape. Internal dry-run/smoke/test signals are hidden from the formal signal flow with truthful source-state handling.
- Data / endpoint behavior: no endpoint contract changed. The page still reads existing authenticated read-only `themes`, `companies`, `signals`, and `strategy/ideas` endpoints only. No RSS/news/commercial-data feature, AI-generated news, live submit, KGI SDK/write-side, Railway secret, migration 0020, destructive DB action, or broker path was touched.
- Tests: `git diff --check -- apps/web/app/themes/[short]/page.tsx apps/web/app/globals.css` PASS with CRLF warnings only; changed-file bad-character scan found 0 replacement/private-use characters; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Local QA: local production-server smoke on `/themes/ai` at 1365px and 390px with a middleware-only session plus temporary read-only fixture API returned status 200, 0 console errors, 0 page errors, 0 horizontal overflow, 0 replacement/private-use bad characters, 0 English-heavy blocks, and 0 narrow vertical text stacks. Evidence is local at `evidence/w7_paper_sprint/local_visual_qa_pass99_theme_detail_2026-05-05/`. Playwright recorded only normal Next.js prefetch `ERR_ABORTED` navigation noise.
- Blockers / bypass: production PR/CI/deploy and post-deploy smoke are next. KGI `libCGCrypt.so` remains the only live-submit blocker; Taiwan unit safety remains unchanged (`零股` = actual shares, `整張` = 1,000 shares).

## Codex Cycle (2026-05-05 04:22 Taipei) - strategy-runs purpose and layout repair
- Files changed: updated `apps/web/app/runs/page.tsx`, `apps/web/app/runs/[id]/page.tsx`, `apps/web/app/globals.css`, and this board.
- Behavior: rebuilt `策略批次` from a raw table into a route-specific batch monitor. The list now explains that each batch is a strategy-screen snapshot, not an order entry page, and renders each batch as a card with generated time, mode, 可觀察 / 待審 / 阻擋 counts, quality summary, and top candidate reason in Traditional Chinese.
- Behavior: repaired `策略批次明細` so it no longer exposes raw `GET /api` source paths, full UUID page titles, English query keys, or machine reasons like `missing_bars`. Query conditions are now Chinese chips, candidate outputs are cards, batch IDs are shortened in layout-sensitive places, and long monospace status text wraps safely on mobile.
- Data / endpoint behavior: no endpoint contract changed. Both pages still read existing authenticated read-only `strategy/runs` endpoints only. Candidate-to-order promotion remains explicitly paused until backend preview contract and risk gates are ready. No live submit, KGI SDK/write-side, Railway secret, migration 0020, destructive DB action, RSS/news/commercial-data feature, AI-generated news, or broker path was touched.
- Tests: `git diff --check -- apps/web/app/runs/page.tsx apps/web/app/runs/[id]/page.tsx apps/web/app/globals.css` PASS with CRLF warnings only; changed-file bad-character scan found 0 replacement/private-use characters; `pnpm.cmd --filter @iuf-trading-room/web build` PASS; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS.
- Local QA: local production-server smoke on `/runs` and `/runs/11111111-1111-4111-8111-111111111111` at 1365px and 390px with a middleware-only session plus temporary read-only fixture API returned status 200, 0 console errors, 0 page errors, 0 horizontal overflow, 0 replacement/private-use bad characters, 0 machine terms, 0 English-heavy blocks, and 0 narrow vertical text stacks. Evidence is local at `evidence/w7_paper_sprint/local_visual_qa_pass100_runs_2026-05-05_final4/`.
- Blockers / bypass: production PR/CI/deploy and post-deploy smoke are next. KGI `libCGCrypt.so` remains the only live-submit blocker; Taiwan unit safety remains unchanged (`零股` = actual shares, `整張` = 1,000 shares).

## Codex Cycle (2026-05-05 04:44 Taipei) - strategy-ideas workbench repair
- Files changed: updated `apps/web/app/ideas/page.tsx`, `apps/web/app/globals.css`, and this board.
- Behavior: rebuilt `策略想法` into a Traditional Chinese台股候選工作台. The route now explains that strategy ideas are read-only candidate observations, not order tickets, and the hero/summary area focuses on 可觀察 / 待審 / 阻擋 counts instead of a generic table.
- Behavior: replaced the old dense strategy idea table with candidate cards that show 代號、公司、方向、分數、信心、訊號數、決策、主題、資料品質、readiness/freshness, and source. `missing_bars` / `insufficient_bars` / raw strategy endpoint text are mapped or hidden; transfer-to-order remains a non-clickable `轉單待接` state.
- Data / endpoint behavior: no endpoint contract changed. The page still reads the authenticated read-only `/api/v1/strategy/ideas` contract through `getStrategyIdeas`. No live submit, KGI SDK/write-side, Railway secret, migration 0020, destructive DB action, RSS/news/commercial-data feature, AI-generated news, or broker path was touched.
- Tests: `git diff --check -- apps/web/app/ideas/page.tsx apps/web/app/globals.css` PASS with CRLF warnings only; changed-file bad-character scan found 0 replacement/private-use characters; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Local QA: local production-server smoke on `/ideas` at 1365px and 390px with a middleware-only session plus temporary read-only fixture API returned status 200, 0 console errors, 0 page errors, 0 horizontal overflow, 0 replacement/private-use bad characters, 0 machine terms, 0 English-heavy blocks, and 0 narrow vertical text stacks. Evidence is local at `evidence/w7_paper_sprint/local_visual_qa_pass101_ideas_2026-05-05_final2/`.
- Blockers / bypass: production PR/CI/deploy and post-deploy smoke are next. KGI `libCGCrypt.so` remains the only live-submit blocker; Taiwan unit safety remains unchanged (`零股` = actual shares, `整張` = 1,000 shares).

## Codex Cycle (2026-05-05 05:04 Taipei) - Quant Lab source-truth surface repair
- Files changed: updated `apps/web/app/lab/LabClient.tsx`, `apps/web/app/globals.css`, and this board.
- Behavior: rebuilt `量化研究` as a strategy-bundle inbox rather than a fake-performance showcase. When the lab API is blocked or returns 0 bundles, the page now shows explicit source/owner/status copy and the exact fields that will appear after backend wiring; it does not show sample Sharpe/equity curves or fake strategy rows.
- Behavior: when real lab bundles exist, the page renders route-specific bundle cards with producer, status, symbol, theme, confidence, return/drawdown summary, Chinese summary cleanup, and explicit `轉單待接`. Green/red semantics were tightened: `通過` is the green primary action, `退回` is red-soft, and broker/portfolio promotion stays non-clickable until the handoff contract exists.
- Data / endpoint behavior: no endpoint contract changed. The page still reads `/api/v1/lab/bundles` through `radarLabApi`; approve/reject buttons remain wired to the existing lab action API only. No live submit, KGI SDK/write-side, Railway secret, migration 0020, destructive DB action, RSS/news/commercial-data feature, AI-generated news, or broker path was touched.
- Tests: `git diff --check -- apps/web/app/lab/LabClient.tsx apps/web/app/globals.css` PASS with CRLF warnings only; changed-file bad-character scan found 0 replacement/private-use characters; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Local QA: local production-server smoke on `/lab` at 1365px and 390px with a middleware-only session plus temporary read-only fixture API returned status 200, 0 console errors, 0 page errors, 0 horizontal overflow, 0 replacement/private-use bad characters, 0 machine terms, 0 English-heavy blocks, and 0 narrow vertical text stacks. Evidence is local at `evidence/w7_paper_sprint/local_visual_qa_pass102_lab_2026-05-05/`.
- Blockers / bypass: production PR/CI/deploy and post-deploy smoke are next. KGI `libCGCrypt.so` remains the only live-submit blocker; Taiwan unit safety remains unchanged (`零股` = actual shares, `整張` = 1,000 shares).

## Codex Cycle (2026-05-05 05:24 Taipei) - Daily Brief data framework repair
- Files changed: updated `apps/web/app/briefs/page.tsx`, `apps/web/app/globals.css`, and this board.
- Behavior: rebuilt `每日簡報` as a Taiwan-market brief framework. The route now has a metric strip, source-truth hero, and explicit AI handoff copy: OpenAlice can summarize only ingested market/company/strategy/risk data, and the UI will not fabricate news or trading advice when the backend has no brief rows.
- Behavior: EMPTY/BLOCKED states now show useful source wiring details instead of a single dead panel: target sections include 盤勢、FinMind/重大訊息、策略候選、風控/交易狀態. LIVE state continues to render official brief sections and history from the existing daily brief DB.
- Data / endpoint behavior: no endpoint contract changed. The page still reads `/api/v1/briefs` through `getBriefs`. No live submit, KGI SDK/write-side, Railway secret, migration 0020, destructive DB action, RSS/news/commercial-data feature, AI-generated news, or broker path was touched.
- Tests: `git diff --check -- apps/web/app/briefs/page.tsx apps/web/app/globals.css` PASS with CRLF warnings only; changed-file bad-character scan found 0 replacement/private-use characters; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Local QA: local production-server smoke on `/briefs` at 1365px and 390px with a middleware-only session plus temporary read-only fixture API returned status 200, 0 console errors, 0 page errors, 0 horizontal overflow, 0 replacement/private-use bad characters, 0 machine terms, 0 English-heavy blocks, and 0 narrow vertical text stacks. Evidence is local at `evidence/w7_paper_sprint/local_visual_qa_pass103_briefs_2026-05-05/`.
- Blockers / bypass: production PR/CI/deploy and post-deploy smoke are next. AI daily narrative generation remains intentionally deferred until backend source bundles and OpenAlice generation contract exist; KGI `libCGCrypt.so` remains the only live-submit blocker; Taiwan unit safety remains unchanged (`零股` = actual shares, `整張` = 1,000 shares).

## Codex Cycle (2026-05-05 05:58 Taipei) - quote and order-room compact repair
- Files changed: updated `apps/web/app/quote/page.tsx`, `apps/web/components/portfolio/OrderTicket.tsx`, `apps/web/app/globals.css`, and this board.
- Behavior: `/quote?symbol=2330` now uses the same FinMind Sponsor minute-K contract as the company page. Daily/weekly/monthly K remains based on formal OHLCV, while 1/5/15/60 minute K is supplied through `getCompanyKBar` and rendered with the shared K-line component. Quote source/freshness labels are Traditional Chinese and empty reason chips no longer render `none`.
- Behavior: the portfolio/company order ticket was tightened from a slabby two-column block into a calmer single-column trading ticket with compact segment controls, price-axis mini K-line, and explicit unit safety. Status semantics remain traffic-light: normal/pass states green, blocked/fail states red, empty/no-data states amber.
- Behavior: global mobile navigation now stays contained inside its own horizontal rail and no longer expands the document width. K-line pending copy can wrap inside the panel instead of clipping on narrow screens.
- Data / endpoint behavior: no endpoint contract changed. This pass only wires existing read-only quote/company K-line APIs and existing paper preview/order surfaces. No live submit, KGI SDK/write-side, Railway secret, migration 0020, destructive DB action, RSS/news/commercial-data feature, AI-generated news, or broker path was touched.
- Tests: `git diff --check -- apps/web/app/globals.css apps/web/app/quote/page.tsx apps/web/components/portfolio/OrderTicket.tsx` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Local QA: `/quote?symbol=2330` and `/portfolio` at 1365px and 390px with a middleware-only local session plus a temporary read-only fixture API returned status 200, 0 console errors, 0 page errors, 0 failed responses, 0 document horizontal overflow, 0 replacement/private-use bad characters, 0 English-heavy terms, and 0 narrow vertical text stacks. Evidence is local at `evidence/w7_paper_sprint/local_visual_qa_pass104_quote_order_2026-05-05_final_auth5/`.
- Blockers / bypass: production PR/CI/deploy and post-deploy smoke are next. KGI `libCGCrypt.so` remains the only live-submit blocker; Taiwan unit safety remains unchanged: odd lot is actual shares, board lot is 1,000 shares.

## Codex Cycle (2026-05-05 06:31 Taipei) - company financial console density repair
- Files changed: updated `apps/web/app/companies/[symbol]/FinancialsPanel.tsx`, `apps/web/app/globals.css`, and this board.
- Behavior: tightened the company page `財報與估值` console so FinMind tabs no longer become oversized blocks or push the page down with long summaries. Each dataset tab now shows a compact state (`讀取中` / `暫停` / `無資料` / `N 筆`), and the active dataset header says the table is fixed at 10 rows per page.
- Behavior: constrained the financial table shell with a fixed-height scrolling region plus pagination, reduced cell padding, compacted metric cards, and kept source/status/freshness visible without the giant slab effect. This directly addresses the user complaint that `市值` / `估值` expansion made the whole company page ugly and left too much blank space.
- Data / endpoint behavior: no endpoint contract changed. The page still reads existing FinMind-backed company financial endpoints only. No live submit, KGI SDK/write-side, Railway secret, migration 0020, destructive DB action, RSS/news/commercial-data feature, AI-generated news, or broker path was touched.
- Tests: `git diff --check -- apps/web/app/companies/[symbol]/FinancialsPanel.tsx apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Local QA: local visual QA on `/companies/2330` at 1365px with a middleware-only session plus temporary read-only fixture API showed 0 document horizontal overflow and 0 replacement/private-use bad characters; after fixture endpoint completion, rerun had 0 bad responses. Evidence is local at `evidence/w7_paper_sprint/local_visual_qa_pass105_company_finance_2026-05-05/` and `_rerun/`. Dev-only RSC mismatch noise was observed during hot reload, but production build passed.
- Blockers / bypass: production PR/CI/deploy and post-deploy smoke are next. Broader visual backlog remains: company workbench/order layout aesthetics, dashboard route-specific identity, and FinMind Sponsor dataset expansion. KGI `libCGCrypt.so` remains the only live-submit blocker; Taiwan unit safety remains unchanged: `零股` = actual shares, `整張` = 1,000 shares.

## 2026-05-05 07:12 Taipei - Codex pass106 company workbench repair (IN PROGRESS)
- Files changed: `apps/web/app/companies/[symbol]/page.tsx`, `apps/web/app/globals.css`, this board.
- Behavior changed: company detail now wraps K-line and the simulated order ticket in `company-workbench-shell`. At 1365px the K-line stays the dominant workspace, while the order ticket moves below it as a compact two-column station instead of squeezing the chart into a narrow side rail. On wider desktop the same shell can still place the order station beside the chart; on tablet/mobile it collapses cleanly.
- Layout repaired: K-line control groups were reduced into a compact toolbar; snapshot cells now sit in smaller data chips; chart shell at 1365px measured ~894px wide with no horizontal overflow. The paper ticket keeps `零股` default, makes `整張` explicit, keeps price/quantity controls compact, and puts preview/ledger in a secondary column instead of stacked slabs.
- Data / stop-line behavior: no endpoint contract changed. No live submit, Railway secret, migration 0020, KGI SDK/broker write-side, destructive DB action, commercial/RSS/news feature, or fake-data path was touched.
- Tests: `git diff --check -- apps/web/app/companies/[symbol]/page.tsx apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web build` PASS; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS after Next regenerated `.next/types`.
- Local QA: restarted local Next dev server with local read-only fixture API at `127.0.0.1:3162`; Playwright 1365px `/companies/2330` screenshot saved at `evidence/w7_paper_sprint/pass106_company_workbench_1365_rerun.png`. Metrics: `overflowX=false`, body `scrollWidth=1365`, K-line panel width `938px`, chart shell width `894px`, toolbar height `45px`.
- Next: commit/push/open PR for pass106, then continue with the next visible backlog item: status color semantics across routes (`正常`/`通過` green, `暫停`/`無法顯示` red) and the remaining order-ticket refinement if production screenshots show residual heaviness.

## 2026-05-05 07:33 Taipei - Codex pass106 merge/deploy + pass107 status semantics
- Merge/deploy: PR #165 `fix(web): reshape company trading workbench` was squash-merged to main as `c85c61b`. Railway web deployment `e1ca84de-8c12-490f-b79a-120aca53bcdf` is `SUCCESS`, so production is no longer stuck on the older company-workbench build.
- Files changed in pass107: `apps/web/app/globals.css`, `apps/web/app/lab/LabClient.tsx`, `apps/web/app/m/kill/page.tsx`, `apps/web/components/portfolio/KillSwitch.tsx`, and this board.
- Behavior: tightened traffic-light semantics after operator feedback. `正常` / `通過` / `可交易` now have explicit green visual treatment in session pills, mobile kill-switch rows, Quant Lab approve buttons, and desktop kill-switch mode tiles. `退回` / `暫停` / `不可用` keep red treatment. Fixed the stale `.danger-soft` CSS variable typo so reject/fail actions no longer lose their red color.
- Data / stop-line behavior: front-end color/layout only. No endpoint contract changed. No live submit, Railway secret, migration 0020 promotion, KGI SDK/broker write-side, destructive DB action, commercial/RSS/news feature, or fake-data path was touched. Taiwan order-unit safety remains unchanged: odd lot is actual shares, board lot is 1,000 shares.
- Tests: `git diff --check -- apps/web/app/globals.css apps/web/app/lab/LabClient.tsx apps/web/app/m/kill/page.tsx apps/web/components/portfolio/KillSwitch.tsx` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Local QA: Playwright 1365px smoke on `/m/kill`, `/lab`, `/portfolio`, and `/companies/2330` against local Next + read-only fixture API returned status 200, no horizontal overflow, and no narrow vertical text stacks. Evidence: `evidence/w7_paper_sprint/local_visual_qa_pass107_status_semantics_2026-05-05/`.
- Next: commit/push/open PR for pass107, then continue with the next visible backlog item: a deeper company/order station aesthetic pass and minute-K production smoke after the current deploy settles.

## 2026-05-05 08:11 Taipei - Codex pass108 company K-line and order controls
- Files changed: `apps/web/app/companies/[symbol]/OhlcvCandlestickChart.tsx`, `apps/web/app/companies/[symbol]/PaperOrderPanel.tsx`, `apps/web/app/globals.css`, and this board.
- Behavior: K-line interval and range controls now expose real selected state through `aria-pressed` and shared active CSS instead of inline-only styling. This makes 1min / 5min / 15min / 60min / daily / weekly switching visibly and programmatically verifiable, and keeps the active tool compact instead of looking like a large page block.
- Behavior: company workbench K-line toolbar and chart shell were lightly polished so the chart remains the main workspace, while the simulated order panel reads more like a trading dock than stacked slabs. The enabled order action now uses green success semantics; blocked/disabled states remain muted or red according to the existing guard state.
- Data / stop-line behavior: front-end state/layout only. No endpoint contract changed. No live submit, Railway secret, migration 0020 promotion, KGI SDK/broker write-side, destructive DB action, commercial/RSS/news feature, or fake-data path was touched. Taiwan order-unit safety remains unchanged: odd-lot is actual shares and board-lot is exactly 1,000 shares.
- Tests: `git diff --check -- apps/web/app/companies/[symbol]/OhlcvCandlestickChart.tsx apps/web/app/companies/[symbol]/PaperOrderPanel.tsx apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Local QA: clean local Next dev server on `127.0.0.1:3155` plus read-only fixture API on `127.0.0.1:3162`; Playwright desktop 1365px and mobile 390px `/companies/2330` clicked 1min, 5min, 15min, 60min, daily, and weekly. Results: 0 console/network errors, 0 horizontal overflow, 0 narrow vertical text stacks, chart rendered for every tested interval, and `aria-pressed` matched the clicked interval. Evidence: `evidence/w7_paper_sprint/local_visual_qa_pass108_kline_order_2026-05-05/qa-result-clean3155.json` plus screenshots in the same folder.
- Tooling note: earlier `next start` and a stale 3158 dev server produced false local errors; the clean 3155 run is the evidence source for this pass.
- Next: commit/push/open PR for pass108, wait for CI, then continue the visible backlog: more route-specific design cleanup after deployment, especially dashboard identity and remaining order-station aesthetics.


## 2026-05-05 08:31 Taipei - Codex pass109 dashboard structure repair
- Files changed: `apps/web/app/page.tsx`, `apps/web/app/globals.css`, and this board.
- Behavior: dashboard no longer collapses into a single blocked summary when several sources are temporarily unavailable. The health summary now renders as an alert band, and the main dashboard still keeps watchlist, themes, ideas, market intel, signals, ops, and runs visible with truthful LIVE / EMPTY / BLOCKED states.
- Behavior: dashboard hero was reshaped into a two-column trading-room command deck on desktop, with source-state rail on the right and compact KPI cells under the hero copy. Mobile now collapses to a single clean column; the old 10px + 320px grid artifact that caused narrow vertical text is removed.
- Data / stop-line behavior: front-end layout/state only. No endpoint contract changed. No live submit, Railway secret, migration 0020 promotion, KGI SDK/broker write-side, destructive DB action, commercial/RSS/news feature, or fake-data path was touched.
- Tests: `git diff --check -- apps/web/app/page.tsx apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Local QA: clean local Next dev server on `127.0.0.1:3155` plus read-only fixture API on `127.0.0.1:3162`; Playwright checked `/` at 1365px and 390px. Final results: 0 console/network errors, 0 horizontal overflow, 0 narrow vertical text stacks, 8 panels retained even when sources are degraded, desktop hero uses two columns, mobile mosaic uses a single 354px column. Evidence: `evidence/w7_paper_sprint/local_visual_qa_pass109_dashboard_2026-05-05/`.
- Next: commit/push/open PR for pass109, then continue route-specific polish after CI/deploy, with company/order station and dashboard production smoke as the immediate next checks.

## 2026-05-05 09:00 Taipei - Codex pass110 company order desk refinement
- Files changed: `apps/web/app/companies/[symbol]/PaperOrderPanel.tsx`, `apps/web/app/globals.css`, and this board.
- Deploy status: prior dashboard PR #168 is deployed on Railway web as SUCCESS at 2026-05-05 08:36 Taipei. This pass starts from `origin/main` after #168.
- Behavior: at 1365px desktop, the company page now keeps K-line and the simulated order ticket in the same trading desk: K-line remains the primary panel, and the order ticket sits as a compact right-side ticket instead of becoming a large slab below the chart. Below tablet width it still collapses to one column.
- Behavior: paper-order controls now have more breathing room and clearer focus states; truth notes gained state classes so LIVE/EMPTY/BLOCKED can be colored consistently. Normal/pass remains green, blocked/fail red, empty/no-data amber. Long company/source text now wraps safely inside company hero/info/source panels instead of spilling out.
- Data / stop-line behavior: front-end layout/state only. No endpoint contract changed. No live submit, Railway secret, migration 0020 promotion, KGI SDK/broker write-side, destructive DB action, commercial/RSS/news feature, or fake-data path was touched. Taiwan order-unit safety remains unchanged: odd-lot is actual shares and board-lot is exactly 1,000 shares.
- Tests: `git diff --check -- apps/web/app/companies/[symbol]/PaperOrderPanel.tsx apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Local QA: clean local Next dev server on `127.0.0.1:3155` plus read-only fixture API on `127.0.0.1:3162`; Playwright checked `/companies/2330` at 1365px and 390px, then clicked the 5-minute K-line tab. Results: 0 console/network errors, 0 horizontal overflow, 0 narrow vertical text stacks, active interval became `5?`, and K-line metadata updated to the intraday series. Evidence: `evidence/w7_paper_sprint/local_visual_qa_pass110_company_order_2026-05-05/qa-result-5min-final.json` plus screenshots in the same folder.
- Next: commit/push/open PR for pass110, wait for CI, merge/deploy if green, then continue with the next repair slice: remaining route-specific visual identity and broader FinMind Sponsor data expansion after stable UI truth surfaces.

## 2026-05-05 09:41 Taipei - Codex pass111 company labels and overflow repair
- Files changed: `apps/web/lib/company-adapter.ts`, `apps/web/app/companies/[symbol]/page.tsx`, `apps/web/app/companies/[symbol]/OhlcvCandlestickChart.tsx`, `apps/web/app/globals.css`, and this board.
- Behavior: company detail no longer exposes raw theme UUIDs in the hero or KPI strip. The page now resolves theme ids through the authenticated themes endpoint when available, cleans `[ORPHAN]` display names, and falls back to `主題待接` instead of showing database identifiers to operators.
- Behavior: at 1365px desktop, the company workbench no longer overflows by combining a fixed K-line width with the right order ticket. The K-line remains the dominant workspace, the simulated order ticket stays as a compact right-side ticket, the K-line source badge wraps safely, and the FinMind financial console now spans the company data row instead of being squeezed into a half-width panel.
- Behavior: the FinMind financial toolbar and tabs were tightened so long notes wrap inside the panel; table/pagination behavior remains capped at 10 rows per page and does not push the whole company page down.
- Data / stop-line behavior: front-end label/layout only. No endpoint contract changed. No live submit, Railway secret, migration 0020 promotion, KGI SDK/broker write-side, destructive DB action, commercial/RSS/news feature, or fake-data path was touched. Taiwan order-unit safety remains unchanged: odd-lot is actual shares and board-lot is exactly 1,000 shares.
- Tests: `git diff --check -- apps/web/lib/company-adapter.ts apps/web/app/companies/[symbol]/page.tsx apps/web/app/companies/[symbol]/OhlcvCandlestickChart.tsx apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Local QA: clean local Next dev server on `127.0.0.1:3155` plus read-only fixture API on `127.0.0.1:3162`; Playwright checked `/companies/2330` at 1365px and clicked `5分`. Results: 0 console errors, 0 failed requests, 0 document horizontal overflow, 0 element overflows after the toolbar wrap fix, 0 UUIDs in visible text, K-line source badge contained, finance panel width `938px`, and active interval became `5分` with metadata `1 分 K 彙整48 根`. Evidence: `evidence/w7_paper_sprint/local_visual_qa_pass111_company_layout_2026-05-05/qa-result-final.json` plus day/5-minute screenshots.
- Next: commit/push/open PR for pass111, wait for CI, merge/deploy if green, then continue the next repair slice: broader route-specific identity and FinMind Sponsor dataset expansion after production confirms this company-page cleanup.

## 2026-05-05 10:18 Taipei - Codex pass112 company order unit presets
- Files changed: `apps/web/app/companies/[symbol]/PaperOrderPanel.tsx`, `apps/web/app/globals.css`, and this board.
- Behavior: the company simulated order ticket now exposes compact quantity presets tied to the selected Taiwan unit. Odd-lot mode offers `1 / 10 / 100 / 499 / 999 股`; board-lot mode offers `1 / 2 / 5 張` and immediately shows a truth note that board-lot mode calculates as `1,000 股` per lot. This keeps high-price testing safe by making odd-lot vs board-lot visible instead of implicit.
- Behavior: the preset row is styled as a small trading-tool strip inside the existing ticket instead of another large block, preserving the 328px right-side ticket width and avoiding the slab-heavy look.
- Data / stop-line behavior: front-end unit-safety/layout only. No endpoint contract changed. No live submit, Railway secret, migration 0020 promotion, KGI SDK/broker write-side, destructive DB action, commercial/RSS/news feature, or fake-data path was touched. Taiwan order-unit safety is explicit: `零股` = actual shares; `整張` = 1,000 shares.
- Tests: `git diff --check -- apps/web/app/companies/[symbol]/PaperOrderPanel.tsx apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Local QA: after clearing a stale `.next` dev-cache error, clean local Next dev server on `127.0.0.1:3155` plus read-only fixture API on `127.0.0.1:3162`; Playwright checked `/companies/2330` at 1365px, clicked `5分`, `100 股`, `整張`, and `1 張`. Results: 0 console errors, 0 page errors, 0 failed responses, 0 horizontal overflow, active K-line `5分`, quick-row visible, active preset `1 張`, and board-lot conversion note visible. Evidence: `evidence/w7_paper_sprint/local_visual_qa_pass112_order_unit_presets_2026-05-05/qa-result-selector.json` plus screenshots in the same folder.
- Next: commit/push/open PR for pass112, wait for CI, merge/deploy if green, then continue the next repair slice: make the order ticket visually lighter and start the broader company-page route-specific polish without changing broker/write-side behavior.

## 2026-05-05 10:43 Taipei - Codex pass113 order ticket density polish
- Deploy follow-up: PR #171 was squash-merged as `aba425e`; Railway web deploy `41f593ae-c24e-411f-b0a8-0a6cb5782021` is `SUCCESS`. No-auth production smoke on `/login` and protected `/companies/2330` redirect returned 0 console errors, 0 page errors, and 0 failed responses. Evidence: `evidence/w7_paper_sprint/production_smoke_pass112_2026-05-05/`.
- Files changed: `apps/web/app/globals.css`, this board, and pass113 QA evidence.
- Behavior: the company simulated order ticket now reads more like a trading tool instead of a heavy block. At 1365px the right ticket expands from the cramped 328px lane to a 368px lane while keeping the chart primary; the ticket gets a lighter trading-desk background, no inner ticket border, smaller warning strip, tighter price row, compact summary chips, and an inline quick-preset row.
- Behavior: the board-lot/odd-lot guard from pass112 remains unchanged. The visual change only makes the unit controls and risk note easier to scan; it does not alter order payloads, preview/submit APIs, or broker paths.
- Data / stop-line behavior: front-end CSS/layout only. No endpoint contract changed. No live submit, Railway secret, migration 0020 promotion, KGI SDK/broker write-side, destructive DB action, commercial/RSS/news feature, or fake-data path was touched.
- Tests: `git diff --check -- apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Local QA: after clearing stale dev `.next`, Playwright checked `/companies/2330` at 1365px and 390px, clicked `5分` and `整張`. Final results: 0 console errors, 0 page errors, 0 failed responses, 0 document horizontal overflow, 0 narrow vertical text stacks, active K-line `5分`, order panel width `368px` desktop / `354px` mobile, quick row visible, and board-lot conversion note visible. Evidence: `evidence/w7_paper_sprint/local_visual_qa_pass113_order_ticket_density_2026-05-05/qa-result-final.json`.
- Next: commit/push/open PR for pass113, wait for CI, merge/deploy if green, then continue the broader visual backlog: dashboard route identity and company data panels without changing the truthful 4-state data rules.

## 2026-05-05 11:01 Taipei - Codex pass114 dashboard identity repair
- Deploy follow-up: PR #172 was squash-merged as `e164956`; Railway web deploy `ce06cba5-698c-4733-b060-2a33339f1b6b` is `SUCCESS`. No-auth production smoke on `/login` and protected `/companies/2330` redirect returned 0 console errors, 0 page errors, and 0 failed responses. Evidence: `evidence/w7_paper_sprint/production_smoke_pass113_2026-05-05/`.
- Files changed: `apps/web/app/globals.css`, this board, and pass114 dashboard QA evidence.
- Behavior: the dashboard hero now reads more like a Taiwan-stock command deck instead of a generic stack of blocks. It keeps the same truthful source statuses, but the hero gets a subtle grid/radar surface, compact KPI cells, and a tighter source rail so blocked sources remain visible without taking over the whole first screen.
- Behavior: FinMind Sponsor readiness and the degraded-source summary were compressed into scan-friendly bands. LIVE / EMPTY / BLOCKED / HIDDEN semantics remain intact: green means available/pass, red means blocked/fail, amber means no-data/pending. The UI still says when backend data is unavailable instead of filling gaps with fake rows.
- Behavior: mobile dashboard no longer produces narrow vertical text stacks in the source/major-news sections. The source rail uses a two-column compact layout on small screens, and the degraded-source details wrap normally instead of squeezing labels against borders.
- Data / stop-line behavior: front-end CSS/layout only. No endpoint contract changed. No live submit, Railway secret, migration 0020 promotion, KGI SDK/broker write-side, destructive DB action, commercial/RSS/news feature, or fake-data path was touched.
- Tests: `git diff --check -- apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Local QA: Playwright checked `/` at 1365px and 390px with local session cookie plus read-only fixture API. Final results: status 200, 0 console errors, 0 page errors, 0 failed responses, 0 horizontal overflow, 0 replacement/private-use bad characters, 0 English route-title regressions, and 0 narrow vertical text stacks. Evidence: `evidence/w7_paper_sprint/local_visual_qa_pass114_dashboard_identity_2026-05-05_rerun/qa-result.json`.
- Next: commit/push/open PR for pass114, wait for CI, merge/deploy if green, then continue the repair queue: remaining dashboard visual polish from production screenshots, company data panel density, and FinMind Sponsor dataset expansion only after the no-fake UI surface stays stable.

## 2026-05-05 11:31 Taipei - Codex pass115 company financial toolbar density
- Deploy follow-up: PR #173 was squash-merged as `5036c94`; Railway web deploy `2c568418-ffc4-4676-aa89-e51c54b60459` is `SUCCESS`. No-auth production smoke on `/login` and protected `/` redirect returned 0 console errors, 0 page errors, and 0 failed responses. Evidence: `evidence/w7_paper_sprint/production_smoke_pass114_2026-05-05/`.
- Files changed: `apps/web/app/globals.css`, this board, and pass115 company-finance QA evidence.
- Behavior: the company page FinMind financial dataset tabs now behave like a compact data toolbar instead of two rows of oversized blocks. Desktop tab height dropped from the baseline 101px to 42px, while `財報 / 月營收 / 資產負債 / 現金流 / 估值 / 市值 / 股利` remain visible as real dataset controls.
- Behavior: the financial table shell now keeps a fixed-height scroll region and compact pagination. Switching into the high-row `估值` and `市值` datasets keeps the table at roughly 396px on desktop and 398px on mobile instead of pushing the whole company page far downward.
- Behavior: source, freshness, LIVE/EMPTY/BLOCKED color semantics, and 10-row pagination remain visible. This is layout density only; no data is hidden as fake success and no dataset contract changed.
- Data / stop-line behavior: front-end CSS/layout only. No endpoint contract changed. No live submit, Railway secret, migration 0020 promotion, KGI SDK/broker write-side, destructive DB action, commercial/RSS/news feature, or fake-data path was touched. Taiwan order-unit safety remains unchanged: odd-lot is actual shares and board-lot is exactly 1,000 shares.
- Tests: `git diff --check -- apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Local QA: clean local Next dev server on `127.0.0.1:3155` plus read-only fixture API on `127.0.0.1:3162`; Playwright checked `/companies/2330` at 1365px and 390px, then clicked `估值` and `市值`. Final results: status 200, 0 console errors, 0 page errors, 0 failed responses, 0 document horizontal overflow, 0 replacement/private-use bad characters, 0 content overflows outside expected internal scroll rails, and 0 narrow vertical text stacks. Evidence: `evidence/w7_paper_sprint/local_visual_qa_pass115_company_finance_toolbar_2026-05-05/qa-result-final.json` plus desktop/mobile screenshots.
- Next: commit/push/open PR for pass115, wait for CI, merge/deploy if green, then continue the repair queue: company page remaining source panels and dashboard visual warmth before new FinMind Sponsor dataset expansion.

## 2026-05-05 11:52 Taipei - Codex pass116 company source/profile density
- Deploy follow-up: PR #174 was squash-merged as `ec05f1d`; Railway web deploy `ee51119c-f7d9-4477-a272-a7837d4fc10b` is `SUCCESS`. No-auth production smoke on `/login` and protected `/companies/2330` redirect returned 0 console errors, 0 page errors, and 0 failed responses. Evidence: `evidence/w7_paper_sprint/production_smoke_pass115_2026-05-05/`.
- Files changed: `apps/web/app/globals.css`, this board, production pass115 smoke evidence, and pass116 local visual QA evidence.
- Behavior: the company page `公司主檔` and `來源狀態` panels now read like a compact instrument strip under the K-line/order workbench instead of two heavy generic cards. Desktop uses a tighter two-column layout; company identity, market/country/industry/tier, exposure meters, validation placeholders, and source-state rows have less dead air and clearer scan rhythm.
- Behavior: source-state truth remains explicit: green `正常`, amber `過期/無資料`, red `暫停`. No fake source success is introduced; blocked/empty states still show why the data is unavailable.
- Data / stop-line behavior: front-end CSS/layout only. No endpoint contract changed. No live submit, Railway secret, migration 0020 promotion, KGI SDK/broker write-side, destructive DB action, commercial/RSS/news feature, or fake-data path was touched. Taiwan order-unit safety remains unchanged: odd-lot is actual shares and board-lot is exactly 1,000 shares.
- Tests: `git diff --check -- apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Local QA: clean local Next dev server on `127.0.0.1:3155` plus read-only fixture API on `127.0.0.1:3162`; Playwright checked `/companies/2330` at 1365px and 390px. Final results: status 200, 0 console errors, 0 page errors, 0 failed responses, 0 document horizontal overflow, 0 replacement/private-use bad characters, 0 content overflows outside expected internal scroll rails, and 0 narrow vertical text stacks. Evidence: `evidence/w7_paper_sprint/local_visual_qa_pass116_company_source_density_2026-05-05/qa-result.json` plus desktop/mobile screenshots.
- Next: commit/push/open PR for pass116, wait for CI, merge/deploy if green, then continue with remaining page-specific polish. After UI truth/spacing stabilizes, start a separate FinMind Sponsor dataset expansion plan instead of mixing new data work into repair commits.

## 2026-05-05 12:52 Taipei - Codex pass117 company data dock and FinMind ready color
- Deploy follow-up: PR #175 was squash-merged as `eac63ed`; Railway web deploy `d0074693-b2c8-4949-abb2-bffb3126bd9b` is `SUCCESS`. No-auth production smoke on `/login` and protected `/companies/2330` redirect returned 0 console errors, 0 page errors, and 0 failed responses. Evidence: `evidence/w7_paper_sprint/production_smoke_pass116_2026-05-05/`.
- Files changed: `apps/web/app/companies/[symbol]/page.tsx`, `apps/web/app/companies/[symbol]/ChipsPanel.tsx`, `apps/web/app/companies/[symbol]/AnnouncementsPanel.tsx`, `apps/web/app/companies/[symbol]/DerivativesPanel.tsx`, `apps/web/app/companies/[symbol]/TickStreamPanel.tsx`, `apps/web/app/globals.css`, this board, production pass116 smoke evidence, and pass117 visual QA evidence.
- Behavior: the lower company page no longer stacks four more heavy generic cards under the workbench. It now renders as a compact `公司資料艙`: finance stays the primary dataset console, chips and announcements become a right-side source rail, and derivatives/tick readiness become a smaller status rail. This keeps real FinMind data visible without creating another wall of boxes.
- Behavior: the company announcements row now gives the title a real minimum width in the narrow side rail, so公告文字 no longer collapses into a 40px vertical stack. The dock passed 1365px and 390px layout checks with no narrow text columns.
- Behavior: dashboard FinMind `已接資料` tokens now use green operational color (`--tw-dn-bright`) instead of Taiwan price-up red. Price movement colors remain unchanged; this is only for source/readiness semantics.
- Data / stop-line behavior: front-end layout/state/color only. No endpoint contract changed. No live submit, Railway secret, migration 0020 promotion, KGI SDK/broker write-side, destructive DB action, commercial/RSS/news feature, or fake-data path was touched. Taiwan order-unit safety remains unchanged: odd-lot is actual shares and board-lot is exactly 1,000 shares.
- Tests: `git diff --check -- apps/web/app/companies/[symbol]/page.tsx apps/web/app/companies/[symbol]/ChipsPanel.tsx apps/web/app/companies/[symbol]/AnnouncementsPanel.tsx apps/web/app/companies/[symbol]/DerivativesPanel.tsx apps/web/app/companies/[symbol]/TickStreamPanel.tsx apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Local QA: clean dev Next on `127.0.0.1:3052` plus read-only fixture API on `127.0.0.1:3999`; Playwright checked `/companies/1101` at 1365px and 390px. Final results: 0 console errors after filtering known fixture-only missing endpoints, 0 page errors, 0 failed requests, 0 non-fixture bad responses, 0 document horizontal overflow, 0 replacement/private-use bad characters, and 0 narrow vertical text stacks. Evidence: `evidence/w7_paper_sprint/local_visual_qa_pass117_company_data_dock_2026-05-05/qa-result.json` plus desktop/mobile screenshots.
- Fixture note: local fake API still does not implement `balance-sheet`, `cash-flow`, `market-value`, `valuation`, and `shareholding`, so those are recorded as `knownFixtureMissing` only; production uses the real API contracts.
...
## 2026-05-05 19:35 Taipei - Codex production OpenAlice / FinMind freshness smoke
- Files changed: `apps/web/app/page.tsx`, `evidence/w7_paper_sprint/production_smoke_openalice_status_2026-05-05.md`, and this board.
- Behavior: production authenticated route smoke confirms current frontend pages are reachable and `/briefs` truthfully shows stale daily-brief content. OpenAlice observability is healthy, but formal daily brief data remains old: `/api/v1/briefs` returns 2 rows and latest date is `2026-04-25`; `/api/v1/openalice/observability` reports worker/sweep healthy with a 2026-05-05 sweep. Root cause is therefore backend publishing freshness, not a frontend cache-only issue.
- Behavior: dashboard copy was tightened to avoid automated no-fake smoke false positives. It now says missing data remains a documented gap instead of using wording that could be misread as fake/live text.
- Data / stop-line behavior: frontend copy and evidence only. No endpoint contract changed. No live submit, Railway secret, migration 0020 promotion, KGI SDK/broker write-side, destructive DB action, commercial/RSS/news feature, or fake-data path was touched.
- Tests: `git diff --check -- apps/web/app/page.tsx` PASS with CRLF warning only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS; targeted grep for `FakeLive`, `fake.*live`, `mock.*live`, and `假資料補成正常` in changed frontend files returned no matches.
- Production QA: authenticated smoke at 2026-05-05 19:23 Taipei checked `/`, `/briefs`, `/companies/2330`, `/market-intel`, `/signals`, `/themes`, `/ideas`, `/runs`, and `/lab`; all returned 200. Evidence: `evidence/w7_paper_sprint/production_smoke_openalice_status_2026-05-05.md`.
- Next: commit/push/open PR for the copy/evidence patch, then continue with the next safe slice: a read-only frontend status surface for OpenAlice once Jason exposes a daily-brief status endpoint, plus further company/dashboard visual QA only on latest `origin/main`.

## 2026-05-05 19:55 Taipei - Codex `/briefs` OpenAlice jobs visibility
- Files changed: `apps/web/app/briefs/page.tsx`, `apps/web/app/globals.css`, `evidence/w7_paper_sprint/production_smoke_openalice_status_2026-05-05.md`, and this board.
- Behavior: `/briefs` now includes a compact `OpenAlice 最近任務` panel powered by the existing read-only `GET /api/v1/openalice/jobs` endpoint. It lists the latest visible jobs with task type, status, created/updated time, attempt count, and error text if present. This explains why the daily brief is old without pretending a fresh formal row exists.
- State semantics: OpenAlice worker/sweep health remains separate from formal daily brief freshness. A healthy worker is not treated as a current brief; stale `2026-04-25` brief data remains marked stale until the backend publishes a new source-traced row.
- Data / stop-line behavior: frontend read-only rendering only. No review/publish/reject action, no endpoint mutation, no live submit, no Railway secret, no migration 0020 promotion, no KGI SDK/broker write-side, no destructive DB action, and no fake AI report was introduced.
- Tests: `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS; `git diff --check` PASS with CRLF warnings only.
- Next: push the updated PR #189 branch and wait for CI. After merge/deploy, run authenticated production smoke on `/briefs` to confirm the jobs panel renders against real production jobs.

## 2026-05-05 20:06 Taipei - Codex `/briefs` daily brief draft gate
- Deploy follow-up: PR #190 was squash-merged as `1c14529`; GitHub web deploy completed successfully. Authenticated production smoke on `/briefs` confirmed the OpenAlice jobs panel renders in production, still marks the latest formal brief date `2026-04-25` as expired, and showed no token/password/JWT leak.
- Files changed: `apps/web/app/briefs/page.tsx`, `apps/web/app/globals.css`, `evidence/w7_paper_sprint/production_smoke_openalice_status_2026-05-05.md`, and this board.
- Behavior: `/briefs` now adds `每日簡報草稿閘門`, a read-only panel powered by `GET /api/v1/content-drafts?status=awaiting_review&limit=100`. The frontend filters `targetTable === "daily_briefs"` locally because the backend query parameter does not reliably narrow the result set today.
- State semantics: formal brief freshness remains based only on `/api/v1/briefs`; OpenAlice job health remains based on `/api/v1/openalice/jobs`; draft review visibility is based on `/api/v1/content-drafts`. A draft-ready job or awaiting-review draft is not treated as a published current brief.
- Data / stop-line behavior: frontend read-only rendering only. No draft approve/reject/publish action, no endpoint mutation, no live submit, no Railway secret, no migration 0020 promotion, no KGI SDK/broker write-side, no destructive DB action, no fake AI report, and no buy/sell wording was introduced.
- Tests: `git diff --check -- apps/web/app/briefs/page.tsx apps/web/app/globals.css` PASS with CRLF warnings only; `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS; `pnpm.cmd --filter @iuf-trading-room/web build` PASS.
- Next: commit/push/open PR for the draft gate, wait for CI, then run authenticated production smoke on `/briefs` after deployment. After this slice lands, continue the UI repair queue from latest main: authenticated dashboard source-state color check, company K-line minute availability/density, and FinMind Sponsor dataset expansion plan without changing paper/live gates.

## 2026-05-05 22:42 Taipei - Codex K-line incident fix branch
- Trade Capability Score: +1. Workflow improved: `/companies/2330` K-line can recover real FinMind OHLCV instead of staying empty behind cached mock rows.
- Production incident evidence: `evidence/w7_paper_sprint/kline_incident_production_2026-05-05/manifest.md`. The page returned 200 but had no chart shell/canvas; OHLCV endpoint returned 200 rows with non-mock rows = 0; KBar returned EMPTY.
- Root cause: production logs showed FinMind `TaiwanStockPriceAdj` / `TaiwanStockPrice` calls with `end_date=2026-05-05` returning HTTP 400, and `companies-ohlcv` returned all-mock Redis cache before retrying FinMind. This is a data retrieval incident, not a frontend drawing-only bug.
- Files changed: `apps/api/src/data-sources/finmind-client.ts`, `apps/api/src/companies-ohlcv.ts`, `apps/api/src/jobs/ohlcv-finmind-sync.ts`, `apps/api/src/server.ts`, and the K-line incident manifest.
- State semantics: frontend still refuses to render `source="mock"` as a live chart. The fix makes backend latest queries omit `end_date` unless `to` is explicit and prevents mock cache from blocking token-backed Taiwan daily retries.
...
## 2026-05-06 00:22 Taipei - Codex multi-day minute K + deploy unblock
- Trade Capability Score: +1. Workflow improved: company/quote K-line now requests up to 5 recent FinMind trading days, so 1/5/15/60-minute charts are backed by enough rows instead of exposing a thin single-day chart.
- Files changed: `apps/api/src/server.ts`, `apps/web/lib/api.ts`, `apps/web/app/companies/[symbol]/page.tsx`, `apps/web/app/quote/page.tsx`, `apps/web/app/companies/[symbol]/OhlcvCandlestickChart.tsx`, `apps/web/app/globals.css`, `tests/ci.test.ts`, and this board.
- Endpoints/sources: `GET /api/v1/companies/:id/kbar?date=YYYY-MM-DD&days=5` reads FinMind Sponsor KBar only, returns `dateRange`, `daysRequested`, `daysReturned`, and sorted 1-minute rows. It remains read-only and display-only.
- Behavior: company workbench now keeps K-line and simulated order desk in one vertical workflow; the order desk no longer sits beside the chart at cramped widths or covers interval/range buttons. The order form remains paper-only with odd-lot/board-lot safety.
- CI unblock: R17 Case 6 now uses a deterministic outside-hours window based on current Taipei time, removing the midnight flake that blocked the previous main deploy while preserving the `trading_hours` hard-block assertion.
...
## Codex Update - 2026-05-06 23:08 +08:00 - OpenAlice pipeline observability UI
- Trade Capability Score: +1. Workflow improved: `/briefs` now exposes OpenAlice autonomous daily pipeline lifecycle fields so stale or missing daily briefs can be traced without fake publishing.
- Files changed: `apps/web/lib/api.ts`, `apps/web/app/briefs/page.tsx`, `evidence/w7_paper_sprint/codex_openalice_pipeline_observability_ui_2026-05-06.md`, and this board.
- Endpoint/source: `GET /api/v1/openalice/observability` with backend `pipeline` addendum from #230.
...
## 2026-05-07 06:10 TPE - Codex OpenAlice daily brief publish status repair

- Branch: `fix-openalice-brief-published-status-2026-05-07`
- Trade Capability Score: `+1`
- Files: `apps/api/src/content-draft-store.ts`, `apps/worker/src/jobs/daily-brief-producer.ts`, `packages/domain/src/postgres-repository.ts`, `evidence/w7_paper_sprint/codex_openalice_brief_publish_status_repair_2026-05-07.md`
- Behavior: OpenAlice-approved daily briefs and worker fallback formal briefs now surface as `published` through `GET /api/v1/briefs`; legacy `approved` rows and legacy worker `draft` rows are normalized at the repository boundary so `/briefs` does not show empty when automation already produced a formal row.
