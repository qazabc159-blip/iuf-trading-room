# CODEX_PLANS_TRADING_ROOM_BOUNDARY_PR_2026-05-16

## Scope
- Updated `/plans` read-only boundary copy.
- Replaced the stale statement that paper preview/submit lives on the individual stock page.
...
- Static scan:
  - `rg -n "個股頁|模擬預覽、風控核對與委託送出集中在交易室" apps/web/app/plans/page.tsx`
- Local route smoke:
  - started Next dev on `127.0.0.1:3028`
  - `GET /plans` -> served `/login?next=%2Fplans` because the route is auth-gated in this local session
