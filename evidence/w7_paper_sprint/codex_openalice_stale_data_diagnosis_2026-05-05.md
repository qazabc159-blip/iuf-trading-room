...
- Dashboard source rail now includes OpenAlice as a first-class source.
- Dashboard marks OpenAlice red when worker/sweep is stale, missing, or unreadable.
...
- `pnpm.cmd --filter @iuf-trading-room/web typecheck` PASS after dashboard + brief/ops edits.
- `pnpm.cmd --filter @iuf-trading-room/web build` PASS after dashboard + brief/ops edits.
