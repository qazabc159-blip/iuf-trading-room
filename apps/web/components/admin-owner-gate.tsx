...
function gateCopy(state: GateState) {
  if (state.status === "checking") {
    return {
      title: "正在確認 Owner 權限",
      body: "後台頁面只對 Owner 帳號開放，確認完成前不載入內部內容。",
      cta: "請稍候",
      href: "/",
    };
  }
...
function GateShell({ state }: { state: GateState }) {
...
export function AdminOwnerGate({ children }: { children: ReactNode }) {
  ...
  if (state.status !== "ready") return <GateShell state={state} />;
  return <>{children}</>;
}
