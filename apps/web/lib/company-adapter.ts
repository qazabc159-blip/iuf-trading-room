...
export function toCompanyDetailView(company: RadarCompany | ContractCompany | unknown, fallbackSymbol = "2330"): CompanyDetailView {
  ...
  return {
    ...
    notes: stringValue(
      record.notes,
      `${name} 目前放在 ${themes.join(" / ")} 觀察籃。此版骨架財報、籌碼、期權、tick 仍為視覺 placeholder，待 Jim lane 接回真實 endpoint。`,
    ),
...
export function quoteFromOhlcvBars<T extends { dt: string; open: number; close: number; volume: number; source: string }>(
  ...
  if (!bars || bars.length === 0) return toCompanyDetailQuote(null, company);
  ...
  const source = last.source === "kgi" ? "kgi" : last.source === "tej" ? "finmind" : "mock";
  return {
    last: last.close,
    change,
    changePercent,
