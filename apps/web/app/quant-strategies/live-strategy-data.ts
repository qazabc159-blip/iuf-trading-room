...
import { getS1SimBasket, getS1SimStatus, getTrackRecordNav } from "@/lib/fauto-sim-api";
...
export async function loadQuantStrategy(strategyId: string): Promise<QuantStrategy | null> {
  const [snapshot, statusResult, navResult] = await Promise.all([
  ...
  const nav = navResult.ok ? navResult.data : null;

  return getQuantStrategy(strategyId, { snapshot, status, basket, nav });
}
...
export async function loadQuantStrategies(): Promise<QuantStrategy[]> {
  ...
  return QUANT_STRATEGIES.map((strategy) =>
    hydrateQuantStrategy(strategy, { snapshot, status, basket, nav }),
  );
}
