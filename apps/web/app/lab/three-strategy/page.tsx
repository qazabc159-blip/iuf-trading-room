 ...
 *   dm_2026_05_08_athena_yang_truth_board_v1.md
 *
 ...
 *   - caveat 文字顯眼：font-size >= 12, color visible, no opacity < 0.7
 *
...
// Next refresh: 2026-05-15 (or when Athena publishes an updated truth board)

const TRUTH_BOARD_SOURCE = "athena_truth_board_v1 (2026-05-08)";

type GovernanceState =
  | "L9_MARGINAL_PASS"
  | "L8_FAIL_ALL_41_VARIANTS"
  | "KILLED_V11_NO_EDGE";
...
const THREE_STRATEGIES: StrategyCard[] = [
  {
    ...
    badgeLabel: "觀察中",
    badgeColor: "yellow",
    governanceState: "L9_MARGINAL_PASS",
    whatItDoes:
...
function GovernanceBadge({ color, label }: { color: StrategyCard["badgeColor"]; label: string }) {
  const styles: Record<StrategyCard["badgeColor"], React.CSSProperties> = {
    yellow: {
  ...
  return (
    <article
      style={{
        ...
        gap: 14,
        padding: "18px 20px",
        border: `1px solid ${card.badgeColor === "yellow" ? "rgba(255,184,0,0.3)" : card.badgeColor === "red" ? "rgba(220,60,60,0.3)" : "rgba(140,140,140,0.25)"}`,
        borderLeft: `3px solid ${card.badgeColor === "yellow" ? "#ffb800" : card.badgeColor === "red" ? "#e05050" : "#707070"}`,
        borderRadius: 6,
    ...
    >
      ...
      {/* Caveat — prominent, full text, no truncation */}
      <div
        style={{
          padding: "12px 14px",
      ...
      {card.metrics !== null && (
        ...
        >
          <span style={{ fontWeight: 600, color: "#aaa" }}>回測數字狀態：</span>{" "}
          {card.metrics}
        </div>
      )}
    ...
    >
      ...
      >
        ...
        <div style={{ fontSize: 11, color: "#999" }}>
          狀態來源 / {TRUTH_BOARD_SOURCE} ·{" "}
          下次更新 / 2026-05-15 或 Athena 發布新 truth board 時
        </div>
      </section>
      ...
      >
        ...
        >
          ...
          治理資料來源：IUF Quant Lab / Athena truth board v1 /
          dm_2026_05_08_athena_yang_truth_board_v1.md
        </div>
      </Panel>
