...
Additional regression guards:
  11. half-login without IsSucceed never calls show_account
  12. raw AttributeError maps to redacted KGI_LOGIN_OBJECT_MISSING_ATTR
  13. KGI reply strings are redacted before logging
  14. official minimal code-78 path keeps simulation=True and skips account methods
"""
...
# ---------------------------------------------------------------------------
# Test 11: no account method unless login success is positively confirmed
# ---------------------------------------------------------------------------

def test_half_login_without_is_succeed_never_calls_show_account():
    """
    A half-shaped login object must not be treated as success. Even if the
    object carries show_account(), it cannot be called unless IsSucceed is True.
    """
    half_login = MagicMock()
    del half_login.IsSucceed
    half_login.show_account = MagicMock(return_value=[])

    session = KgiSession()
    with patch("kgi_session.kgisuperpy") as mock_kgi:
        mock_kgi.login.return_value = half_login
        with pytest.raises(KgiLoginObjectMissingAttr) as exc_info:
            session.login(person_id="A123456789", person_pwd="wrongpwd", simulation=True)

    assert exc_info.value.attr_name == "IsSucceed"
    half_login.show_account.assert_not_called()
    assert session.is_logged_in is False


# ---------------------------------------------------------------------------
# Test 12: raw AttributeError path is redacted and classified
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Test 13: failed-login reply strings are redacted before logging
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Test 14: official minimal code-78 login result stays classified
# ---------------------------------------------------------------------------

def test_code_78_result_keeps_simulation_true_and_skips_account_methods():
    """
    Preserve the official minimal observation: simulation=True is passed through,
    CA can be OK, and the KGI server may still return code 78. That path is a
    permission/credential rejection and must not call account methods.
    """
    failed_stub = _make_failed_login_result(
        error_code=78,
        reply_string="您尚未申請使用元件，請洽營業員",
    )
    ...
    mock_kgi.login.assert_called_once_with(
        person_id="A123456789",
        person_pwd="wrongpwd",
        simulation=True,
    )
